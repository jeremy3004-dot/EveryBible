...
node --test --experimental-test-module-mocks --import tsx \
  apps/admin/app/serverBoundaryAuth.test.ts \
  apps/admin/app/api/cron/upstream-sync/route.test.ts \
  apps/admin/lib/operator-tools.behavior.test.ts
```

They load the actual server modules through the module loader with mocked auth, response, and data-service boundaries, including synthetic secrets and unknown future fields. `serverBoundaryAuth.test.ts` discovers every server action and route handler, so the engagement refresh and any new handler are covered automatically.
