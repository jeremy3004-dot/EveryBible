...
import { getRecentAuditLogs, getRecentOperatorAuditLogs } from '@/lib/admin-data';
...
export default async function SettingsPage() {
  ...
  const [auditLogs, operatorAuditLogs] = await Promise.all([
    getRecentAuditLogs(50),
    getRecentOperatorAuditLogs(12),
  ]);
      ...
      <section className="card">
        <div className="card__header">
          <div>
            ...
            <h3>Recent OpenClaw actions</h3>
          </div>
        </div>

        {operatorAuditLogs.length === 0 ? (
          <p className="page-copy">
            No OpenClaw mutations have been recorded yet. When the Telegram operator updates the
            homepage or submits a code-change request, the tool, channel, and changed fields will
            appear here.
          </p>
        ) : (
          <div className="table-wrap">
            <table className="data-table">
              ...
              <tbody>
                {operatorAuditLogs.map((log) => (
                  <tr key={log.id}>
                    <td>{formatDateTime(log.created_at)}</td>
                    <td>{log.metadata?.toolName ?? log.action}</td>
                    <td>{log.metadata?.channel ?? 'Unknown'}</td>
                    <td>{log.metadata?.changedFields?.join(', ') || 'None recorded'}</td>
                    <td>{log.summary}</td>
                  </tr>
                ))}
              </tbody>
