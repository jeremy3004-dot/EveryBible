...
export async function verifyCouncilAccess(
  ...
  const lockout = await readPasscodeLockout(service, hash);
  if (lockout === 'unavailable') return unavailable;
  if (lockout === 'locked') return { status: 429, error: 'Too many attempts. Try again later.' };
  ...
  if (mismatch !== 0) {
    return (await recordFailedPasscodeAttempt(service, hash))
