...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let userQuery = supabase.from('user_progress').select('user_id');
    ...
    for (const user of users || []) {
      try {
        ...
        const communityScore = Math.min(
          10,
          Math.floor((((prayersSubmitted || 0) + (annotationsCreated || 0)) / 20) * 10)
        );
        const engagementScore = Math.min(
          100,
          readingScore + listeningScore + streakScore + planScore + communityScore
        );

        // Upsert engagement summary
        const { error: upsertError } = await supabase.from('user_engagement_summary').upsert(
          {
