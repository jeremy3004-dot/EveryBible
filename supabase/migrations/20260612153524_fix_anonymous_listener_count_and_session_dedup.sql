-- =============================================================================
-- Fix 1 — Anonymous listener counts
--   Previously every listenerCount was:
--     COUNT(DISTINCT <alias>.user_id) FILTER (WHERE <alias>.user_id IS NOT NULL ...)
--   This silently drops the ~72% of rows where user_id IS NULL (anonymous
--   sessions), so a location with 1000+ listening minutes could show
--   listenerCount 0.
--
--   Fix: count a distinct person-or-session identifier:
--     COUNT(DISTINCT COALESCE(<alias>.user_id::text, <alias>.session_id))
--     FILTER (WHERE (<alias>.user_id IS NOT NULL OR <alias>.session_id IS NOT NULL)
--             [AND <alias>.listening_minutes > 0])
--
--   Applied to every listenerCount site:
--     * audio_country_rollups
--     * translation_audio_country
--     * location_rollups
--     * translation_location_rollups
--   And also to the scalar userCountWithListening at the final SELECT.
--
--   session_id availability: audio_events already selects event.session_id;
--   geo_activity already passes through audio.session_id and
--   download.session_id — no additional threading was required.
--
-- Fix 2 — Session dedup
--   Authenticated users emit session_started/session_ended through both the
--   authenticated and anonymous code paths, producing two rows with the same
--   session_id (one with user_id, one NULL).  The session_counts CTE already
--   uses COUNT(DISTINCT audio.session_id) FILTER (WHERE session_id IS NOT NULL)
--   which collapses duplicates correctly — no change needed here.
--   totalTrackedSessions is therefore already correct.
-- =============================================================================
