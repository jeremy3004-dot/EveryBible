-- P2 S10: Fix listening-minutes double count in get_admin_analytics_overview.
--
-- The audio_events CTE UNION ALLs audio_completed (minutes from duration_ms) AND
-- audio_playback_progress ticks (minutes from listened_ms). A modern playback
-- emits BOTH, so its minutes were counted twice (live before: 4273 tick minutes
-- + 1324 audio_completed minutes = 5598 reported).
--
-- Progress ticks are now the SINGLE source of listening minutes wherever they
-- exist. audio_completed contributes minutes ONLY for rows predating the first
-- tick (legacy); at/after the tick epoch it is purely a completion marker
-- (0 minutes, but its row still counts the session/user toward listener +
-- session metrics). A DATE cutoff (not session_id) is used deliberately: the
-- historical data emitted audio_completed on the authenticated session_id and
-- ticks on the anonymous session_id, so a session-id join finds no overlap.
-- (Live: 20 legacy rows/31 min predate ticks; 480 rows/1294 min after are the
-- double-counts removed. 5598 -> ~4304.)
--
-- Everything else in the function is unchanged. Per the recurrence guard, this
-- CREATE OR REPLACE is followed by REVOKE ... FROM PUBLIC, anon and a targeted
-- GRANT to service_role (this RPC is admin/server-only).
...
DECLARE
  ...
  -- The moment progress ticks began. audio_completed rows AT/AFTER this are
  -- covered by ticks (which own the minutes); only rows BEFORE it are legacy
  -- minutes. NULL (no ticks anywhere) => count all audio_completed as legacy.
  ...
  RETURN (
    ...
    audio_events AS (
      SELECT
        ...
        -- S10: audio_completed is minutes-authoritative ONLY for legacy rows
        -- predating the first tick; at/after the tick epoch, ticks own the
        -- minutes (avoid double count) and this row is just a completion marker.
        CASE
          WHEN tick_epoch IS NULL OR event.created_at < tick_epoch THEN GREATEST(
    ...
    audio_country_rollups AS (
      SELECT
        ...
        -- FIX 1: count anonymous listeners via session_id fallback
    ...
    location_rollups AS (
      SELECT
        ...
        -- FIX 1: count anonymous listeners via session_id fallback
        COUNT(DISTINCT COALESCE(location.user_id::text, location.session_id))
          FILTER (
            WHERE (location.user_id IS NOT NULL OR location.session_id IS NOT NULL)
              AND location.listening_minutes > 0
          ) AS "listenerCount"
    ...
    SELECT jsonb_build_object(
      ...
      'userCountWithListening', COALESCE(
        (
          SELECT COUNT(DISTINCT COALESCE(audio.user_id::text, audio.session_id))::integer
          FROM audio_events AS audio
          WHERE audio.user_id IS NOT NULL OR audio.session_id IS NOT NULL
        ),
        0
      )
    )
...
-- Recurrence guard: admin/server-only RPC — never PUBLIC/anon/authenticated.
