-- Review A1 (post-P2 hardening): guard client-controlled event_properties numeric
-- casts. The public anon key ships in the app, and track-anonymous-usage-events
-- passes event_properties through untouched — so a single POST with e.g.
-- {"listened_ms":"abc"} would have made every `NULLIF(...)::numeric` cast throw,
-- breaking BOTH get_admin_analytics_overview (admin dashboard) AND the unattended
-- nightly-analytics-maintenance cron (refresh_analytics_monthly_rollup) until the
-- row was hand-deleted. safe_numeric() applies the same regex guard already used
-- for the geo_*_bucket coordinates. Re-creates both functions to use it. The
-- lat/lng bucket extractions were already guarded and are left as-is.
--
-- Verified live: a poison row ({"listened_ms":"abc"}) no longer breaks either
-- function; valid-data totals unchanged (listening 4304, reading 1348,
-- downloads 179).
...
AS $$
  SELECT CASE
    WHEN BTRIM(txt) ~ '^-?[0-9]+(\.[0-9]+)?$' THEN BTRIM(txt)::numeric
    ELSE NULL
  END;
$$;
    ...
    SELECT
      ...
      CASE
        WHEN e.event_name = 'audio_playback_progress'
          THEN GREATEST(COALESCE(public.safe_numeric(e.event_properties->>'listened_ms'), 0), 0) / 60000.0
