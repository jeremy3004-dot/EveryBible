-- Phase 1 (metric truth): give the analytics overview ONE authoritative source
-- for every listener/country count so the client never re-derives them.
--
-- Before this, the admin globe computed "Listeners" by summing the per-bucket
-- locationMetrics (max-merged client-side → 377) while the per-translation table
-- computed listeners with a Math.max accumulator over country rows (→ 381), so a
-- filtered subset could exceed the whole. And the globe's "Countries" counted
-- lat/lng buckets (68) instead of the true ISO country count the RPC already
-- returns as activeCountryCount (12).
--
-- This migration re-creates get_admin_analytics_overview adding two fields; the
-- rest of the function is unchanged (safe_numeric + refresh_analytics_monthly_rollup
-- are untouched):
--   * locatedListenerCount     — DISTINCT COALESCE(user_id, session_id) over
--                                geo-located audio (minutes > 0). The authoritative
--                                "Listeners (located)" shown on the globe. Always
--                                <= userCountWithListening (all-audio total).
--   * translationListenerCounts — per-translation DISTINCT listener count, so the
--                                translation table shows a real dedup count rather
--                                than a max-of-country-rows guess. Each entry is
--                                <= userCountWithListening (subset of all listeners).
--
-- Totals (listening/reading/downloads/users) are unaffected — verified against the
-- pre-migration baseline (listening 4321, reading 1361, downloads 179, users 418).
  ...
  RETURN (
    ...
    audio_events AS (
      SELECT
        ...
        COALESCE(event.geo_latitude, CASE WHEN NULLIF(BTRIM(event.event_properties->>'geo_latitude_bucket'), '') ~ '^-?[0-9]+(\.[0-9]+)?$' THEN (NULLIF(BTRIM(event.event_properties->>'geo_latitude_bucket'), ''))::double precision ELSE NULL END) AS latitude,
        COALESCE(event.geo_longitude, CASE WHEN NULLIF(BTRIM(event.event_properties->>'geo_longitude_bucket'), '') ~ '^-?[0-9]+(\.[0-9]+)?$' THEN (NULLIF(BTRIM(event.event_properties->>'geo_longitude_bucket'), ''))::double precision ELSE NULL END) AS longitude,
      ...
      WHERE event.event_name = 'audio_completed' AND event.created_at >= p_since
      UNION ALL
      SELECT
        event.user_id, event.session_id, event.created_at,
        COALESCE(NULLIF(BTRIM(UPPER(event.geo_country_code)), ''), NULLIF(BTRIM(UPPER(event.event_properties->>'geo_country_code')), '')) AS country_code,
        ...
        COALESCE(event.geo_longitude, CASE WHEN NULLIF(BTRIM(event.event_properties->>'geo_longitude_bucket'), '') ~ '^-?[0-9]+(\.[0-9]+)?$' THEN (NULLIF(BTRIM(event.event_properties->>'geo_longitude_bucket'), ''))::double precision ELSE NULL END) AS longitude,
        NULL AS geo_label,
        GREATEST(COALESCE(public.safe_numeric(event.event_properties->>'listened_ms'), 0), 0) / 60000.0 AS minutes,
        NULLIF(BTRIM(LOWER(event.event_properties->>'translation_id')), '') AS translation_id
      FROM public.analytics_events AS event
      WHERE event.event_name = 'audio_playback_progress' AND event.created_at >= p_since
    ...
    reading_events AS (
      SELECT
        ...
        NULLIF(BTRIM(LOWER(event.event_properties->>'translation_id')), '') AS translation_id
      FROM public.analytics_events AS event
      WHERE event.event_name = 'reading_ended' AND event.created_at >= p_since
    ...
    download_events AS (
      SELECT
        ...
        NULLIF(BTRIM(LOWER(event.event_properties->>'translation_id')), '') AS translation_id
      FROM public.analytics_events AS event
      WHERE event.event_name IN ('text_translation_download_completed', 'audio_download_completed') AND event.created_at >= p_since
    ),
    daily_listening AS (
    ...
    geo_activity AS (
      SELECT audio.user_id, audio.session_id, audio.latitude, audio.longitude, audio.country_code, audio.country_name, audio.geo_label, audio.minutes AS listening_minutes, 0::integer AS download_units, audio.translation_id FROM audio_events AS audio
      UNION ALL
      SELECT download.user_id, download.session_id, download.latitude, download.longitude, download.country_code, download.country_name, download.geo_label, 0::numeric AS listening_minutes, download.download_units, download.translation_id FROM download_events AS download
    ),
    location_rollups AS (
      SELECT CONCAT_WS(':', ROUND(location.latitude::numeric, 2)::text, ROUND(location.longitude::numeric, 2)::text, COALESCE(location.country_code, 'UNSET')) AS key,
      ...
      HAVING COALESCE(SUM(location.listening_minutes), 0) > 0 OR COALESCE(SUM(location.download_units), 0) > 0
    ),
    engagement AS (
    ...
    session_counts AS (
      SELECT COUNT(DISTINCT audio.session_id)::integer AS total FROM audio_events AS audio WHERE audio.session_id IS NOT NULL
    ),
    ...
    translation_listener_totals AS (
      -- Authoritative per-translation distinct-listener count (Phase 1). Replaces
      -- the client-side Math.max over country rows. A subset of all listeners, so
      -- always <= userCountWithListening.
    ...
    SELECT jsonb_build_object(
      ...
      'locationMetrics', COALESCE((SELECT jsonb_agg(jsonb_build_object('key', location.key, 'label', location.label, 'countryCode', location.country_code, 'countryName', location.country_name, 'latitude', location.latitude, 'longitude', location.longitude, 'listeningMinutes', location."listeningMinutes", 'downloadUnits', location."downloadUnits", 'listenerCount', location."listenerCount") ORDER BY location."listeningMinutes" DESC, location."downloadUnits" DESC, location.label) FROM location_rollups AS location), '[]'::jsonb),
