-- Add per-translation analytics to get_admin_analytics_overview.
--
-- Extracts translation_id from event_properties for audio, reading, and
-- download events.  Produces two new JSON arrays:
--
--   translationCountryMetrics  – per-translation country-level rollups
--   translationLocationMetrics – per-translation lat/lng-level rollups
--
-- The admin dashboard uses these to let operators toggle the heatmap and
-- metric cards by individual Bible translation.
...
BEGIN
  RETURN (
    ...
    -- Now includes translation_id from event_properties.
    -- -----------------------------------------------------------------------
    audio_events AS (
      SELECT
        event.user_id,
    ...
    -- -----------------------------------------------------------------------
    -- reading_events: reading_ended events.  Now includes translation_id.
    -- -----------------------------------------------------------------------
    reading_events AS (
      SELECT
        event.user_id,
    ...
    -- -----------------------------------------------------------------------
    -- Existing daily, country, and location rollups (unchanged)
    -- -----------------------------------------------------------------------
    daily_listening AS (
      SELECT
    ...
    -- -----------------------------------------------------------------------
    -- Per-translation country rollups (NEW)
    -- -----------------------------------------------------------------------
    ...
    -- -----------------------------------------------------------------------
    -- Per-translation location rollups (NEW)
    -- -----------------------------------------------------------------------
