...
3. **VOTD empty (CRITICAL health alert since launch)**: the create form depends on `bible_verses` being populated (save action throws "verse is not present in the synced Bible library yet" otherwise). Verify whether `bible_verses` has data; if the save path works, seed the next 7 days of entries as part of verification. Add a "schedule ahead" view showing the next 14 days coverage so empty days are visible before they're live.
