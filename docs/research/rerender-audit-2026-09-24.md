...
- Plans home kept a date bug hidden behind the fresh catalog array. Daily
  ...
  focus, on return to the foreground and at local midnight), so that part was
  not re-applied.
...
| Plans home: memoised cards, Find plans comparator      | Landed differently. The `src/screens/plans/plansHome/` split (another session) memoises `ActivePlanCard`, `CatalogPlanRow` and `RhythmCard` per row, which covers "one plan's progress redraws one card". Catalog rows take the whole progress object, so a `synced_at`-only sync still redraws the synced plan's own row (not all 230 primitives). If that matters, compare the row on `getActivePlanDayNumber(plan, progress, today)` instead of `progress`. The render-count tests on the audit branch (`PlansHomeScreen.render.test.tsx`, "Re-render reach when progress syncs") can be ported to pin it. |
