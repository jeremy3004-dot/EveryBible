...
| 7   | Medium   | `src/services/sync/syncMerge.ts:274`, `syncService.ts:396`                                                     | Same field edited on two offline devices. `synced_at` is the _upload_ time, not the edit time, so an older edit uploaded later beats a newer one; device clock skew decides ties.                                                                                                                                                                                                                                                                                                                   | The newer choice for that one field can lose.                                                                                                                                                                                                                                   | Fixed client-side; needs migration `20260924023259` (per-field `field_updated_at`, not applied). `synced_at` still drives the "Synced N ago" label. |
| 8   | Medium   | `src/services/sync/syncMerge.ts:262-289`                                                                       | First sign-in on a device with guest preferences into an account that already has a client-written preference row. No sync base exists yet, so the whole-row stamp comparison still decides.                                                                                                                                                                                                                                                                                                        | A freshly onboarded device (newer stamp) pushes its untouched defaults over the account's font/theme.                                                                                                                                                                           | Fixed client-side on per-field stamps (defaults carry none); needs migrations `20260924023259` + `20260924023303` (backfill, not applied).          |
| 9   | Medium   | `src/services/plans/readingPlanService.ts` (delete path)                                                       | Device A unenrols (remote delete) while device B, which still has the plan, syncs later.                                                                                                                                                                                                                                                                                                                                                                                                            | B's next push re-creates the row; A's next pull re-enrols A.                                                                                                                                                                                                                    | Fixed client-side; needs migration `20260924023340` (tombstone table + triggers, not applied).                                                      |
...
| 12  | Low      | `src/services/sync/syncService.ts:283-313` (and plans upsert)                                                  | Two devices sync `user_progress` at the same instant: both read R0, both write R0∪own.                                                                                                                                                                                                                                                                                                                                                                                                              | The first writer's new chapters vanish from the server until that device syncs again (local copy still has them).                                                                                                                                                               | `user_progress`: fixed client-side; needs migration `20260924041000` (merge RPC, not applied).                                                      |
...
- ~~`user_reading_plan_progress.completed_sessions jsonb`, `current_session text`~~
  Written as `20260924023342_reading_plan_session_columns.sql` (see below).
- ~~`deleted_at` tombstone on `user_reading_plan_progress` (finding 9).~~ Written as `20260924023340_reading_plan_unenroll_tombstones.sql` (a separate table; see below).
- ~~`edited_at` on `user_preferences` (finding 7).~~ Written as `20260924023259_user_preferences_field_edit_stamps.sql` (see "Server-backed follow-ups" below).
- Atomic merge RPCs for finding 12 are both applied live: plan progress as
  `20260924035821_merge_reading_plan_progress_rpc.sql` and `user_progress.chapters_read`
  as `20260924041000_merge_user_progress_rpc.sql` (see below).
...
`20260924023259_user_preferences_field_edit_stamps.sql` adds
...
- `20260924023303_backfill_user_preferences_field_stamps.sql` stamps rows an app
...
`20260924023340_reading_plan_unenroll_tombstones.sql` adds
...
`20260924023342_reading_plan_session_columns.sql` adds
...
- Two devices writing the same plan row at the same instant: see finding 12
  below.

### Finding 12 (plans): simultaneous pushes keep both phones' days

`20260924035821_merge_reading_plan_progress_rpc.sql` adds
`merge_reading_plan_progress(p_rows jsonb)`, SECURITY INVOKER (own-row RLS and
the tombstone trigger apply), `search_path=''`, EXECUTE for `authenticated`
only. One `INSERT ... ON CONFLICT DO UPDATE` unions `completed_entries` and
`completed_sessions` with the stored row under its row lock and applies the
rest of `mergePlanProgress` (max day, session pointer from the side further
along, completion sticky). The owner is `auth.uid()`; input is validated
(array of at most 100 objects, unique non-blank `plan_slug`, object-valued
maps, else 22023).

- New client pushes (`syncPlanProgress`, `pushProgressToRemote`) call it
  after the usual pre-push read. PGRST202 / 42883 / HTTP 404 falls back to the
  upsert; so does a server whose rows lack the session columns (the function
  needs them). Any other RPC error is a failed push, never a blind upsert.
- Installed builds keep upserting; their read-modify-write race remains until
  they update.

### Order of operations

1. Apply `20260924023259`, `20260924023303`, `20260924023340`,
   `20260924023342` (in that order; each is additive and safe for the
...
### Finding 12 (user_progress): simultaneous pushes keep both devices' chapters

`20260924041000_merge_user_progress_rpc.sql` adds
`merge_user_progress(p_progress jsonb)`, SECURITY INVOKER (own-row RLS applies),
`search_path=''`, EXECUTE for `authenticated` only. It locks the caller's row
(`SELECT ... FOR UPDATE`; a missing row is inserted from the upload) and applies
`mergeReadingSnapshot`'s rules against the stored row: `chapters_read` unioned
with the later timestamp per chapter, the later `last_read_date` with the streak
of the side that owns it (a tie keeps the upload's), and the position read more
recently (stored `synced_at` as the fallback for an unread stored chapter, as on
the client). `synced_at` is the server's `now()`. The owner is `auth.uid()`;
input is validated (object; numeric chapter values under 1-64 character keys, at
most 5000; non-negative integer streak/chapter; `YYYY-MM-DD` date; else 22023).

- `syncProgress` calls it after the usual pre-push read and merges the returned
  row into the local stores, so this device also picks up what another device
  wrote meanwhile. PGRST202 / 42883 / HTTP 404 falls back to the old upsert;
  any other RPC error is a failed push, never a blind upsert.
- Installed builds keep upserting; their read-modify-write race remains until
  they update.
- It is independent of the other migrations and can be applied in any order
  relative to them; it changes no table.
