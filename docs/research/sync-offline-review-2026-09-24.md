...
| 12  | Low      | `src/services/sync/syncService.ts:283-313` (and plans upsert)                                                  | Two devices sync `user_progress` at the same instant: both read R0, both write R0∪own.                                                                                                                                                                                                                                                                                                                                                                                                              | The first writer's new chapters vanish from the server until that device syncs again (local copy still has them).                                                                                                                                                               | `user_progress`: fixed client-side; needs migration `20260924170000` (merge RPC, not applied).                                                      |
...
- Atomic merge RPCs for `user_progress.chapters_read` and plan
  `completed_entries` (finding 12). The `user_progress` half is written as
  `20260924170000_merge_user_progress_rpc.sql` (see below).
...
`20260924170000_merge_user_progress_rpc.sql` adds
