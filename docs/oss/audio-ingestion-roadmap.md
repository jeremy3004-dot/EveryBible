...
- A prior in-repo ingest experiment (the retired `packages/langquest-ingest` package and `apps/workflows` Trigger.dev app) proved out the desired server workflow shape — discover candidates, select translations, copy verified source audio into R2 with immutable cache headers, write per-chapter manifests keyed by canonical book/chapter/verse with SHA-256 checksums, promote an audio manifest, and keep publish state separate from ingestion state. It was removed once Every Language's own distribution replaced it (see `docs/plans/2026-08-03-el-media-integration.md`); the pattern remains the reference for any future server-side importer.
