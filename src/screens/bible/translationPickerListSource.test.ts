...
test('translation picker activates a runtime text translation after download completes', () => {
  ...
  assert.match(
    source,
    /download: \(translation\) => downloadTranslation\(translation\.id\),[\s\S]*activate: \(translation\) => \{[\s\S]*setCurrentTranslation\(translation\.id\);[\s\S]*onRequestClose\?\.\(\);[\s\S]*onTranslationActivated\?\.\(/,
    'Downloaded runtime text translations should become the active Bible once the queued install finishes (ordering is covered by translationPickerDownloadQueue.test.ts)'
  );
});
