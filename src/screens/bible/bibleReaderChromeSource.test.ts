...
test('BibleReaderScreen auto-scrolls inline audio highlights before they leave the viewport', () => {
  ...
  assert.match(
    source,
    /getReaderAutoScrollTarget\(\{[\s\S]*verseOffsetY:\s*verseOffset,[\s\S]*triggerViewportFraction:\s*0\.48,[\s\S]*targetTopOffset:\s*readerContentTopPadding/s,
    'BibleReaderScreen should anchor the active audio verse below the fixed reader header'
  );
  ...
  assert.doesNotMatch(
    source,
    /showPlanSessionChrome && scrollReaderToVerseParagraph\(readerInlineActiveVerse, true\)/,
    'Plan-session audio follow-along should not jump on every verse; it should use the same threshold-based auto-scroll as regular Bible audio'
  );
