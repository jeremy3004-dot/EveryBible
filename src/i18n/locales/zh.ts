export const zh = {
  "readingActivity": {
    "chapters": "章节",
    "currentStreak": "当前连续天数",
    "dayChapters_one": "{{count}} 章 · {{books}}",
    "dayChapters_other": "{{count}} 章 · {{books}}",
    "eyebrow": "更多 · 阅读活动",
    "legendProgress": "{{elapsed}} 天中已读 {{read}} 天",
    "legendRead": "已读",
    "legendToday": "今天",
    "listening": "聆听",
    "nextMonth": "下个月",
    "noReading": "这一天没有阅读记录",
    "noReadingHint": "打开一章，开始填满你的日历。",
    "previousMonth": "上个月",
    "sessionWindow": "{{start}} – {{end}} · {{duration}}",
    "streakUnit_one": "天",
    "streakUnit_other": "天",
  },
  interface: {
    "close": "关闭",
    "share": "分享",
    "minutesShort": "{{count}} 分钟",
    "hoursShort": "{{count}} 小时",
    "daysShort": "{{count}} 天",
    "hoursMinutes": "{{hours}} 小时 {{minutes}} 分钟",
    "justNow": "刚刚",
    "minutesAgo": "{{count}} 分钟前",
    "hoursAgo": "{{count}} 小时前",
    "daysAgo": "{{count}} 天前",
    "activePrayerCount": "{{count}} 项进行中",
    "groupShareMessage": "欢迎加入我在 EveryBible 的门徒训练小组“{{name}}”！\n\n加入代码：{{code}}",
    "groupAboutTitle": "关于小组聚会",
    "groupAboutBody": "小组聚会采用全球门徒训练运动使用的三段式（3/3）模式：",
    "groupLookBack": "你如何顺服了神？与谁分享了？",
    "groupLookUp": "一起阅读并讨论经文",
    "groupLookForward": "你将如何顺服？要告诉谁？",
    "backgroundMusicLabel": "背景音乐：{{name}}",
    "backgroundMusicHint": "打开背景音乐选择器",
    "repeatHint": "在关闭重复、重复本章和重复本卷之间切换",
    "shareAudioHint": "打开本章的音频分享选项",
    "playChapterAudio": "播放本章音频",
    "pauseChapterAudio": "暂停本章音频",
    "previousChapterHint": "前往上一章",
    "chapterRange": "第 {{start}}–{{end}} 章",
    "bookDownloadProgress": "{{completed}}/{{total}} 卷（{{percent}}%）",
    "currentPlanDay": "当前计划第 {{day}} 天",
    "planDay": "第 {{day}} 天",
    "planSessionForDay": "第 {{day}} 天：{{session}}",
    "music": {
      "off": {
        "label": "关闭",
        "description": "播放经文，不添加内置背景音乐。"
      },
      "ambient": {
        "label": "氛围音乐",
        "description": "以循环播放的水下氛围音与铃声衬托朗读。"
      },
      "piano": {
        "label": "钢琴",
        "description": "柔和的钢琴旋律，配以轻盈的背景音。"
      },
      "soft-guitar": {
        "label": "轻柔吉他",
        "description": "不喧宾夺主的尼龙弦吉他循环伴奏。"
      },
      "harp": {
        "label": "竖琴",
        "description": "轻盈的竖琴独奏，营造温柔的灵修氛围。"
      },
      "flute": {
        "label": "长笛",
        "description": "以长笛为主的短循环乐段，带有柔和梦幻的氛围。"
      },
      "sitar": {
        "label": "西塔琴",
        "description": "温暖的沙漠风格西塔琴音色，适合长时间聆听。"
      },
      "ocean-waves": {
        "label": "海浪",
        "description": "循环播放海岸浪声，营造更宁静的背景。"
      }
    },
    "rhythmPresets": {
      "catholic-morning-prayer": {
        "title": "天主教晨祷",
        "description": "以晨祷的形式，在赞美、圣歌与福音祝福中开始一天。",
        "tradition": "天主教",
        "historicRoots": "时辰礼仪"
      },
      "catholic-daytime-prayer": {
        "title": "天主教日间祷告",
        "description": "以简短的圣咏构成午间日课，让心持续归向神。",
        "tradition": "天主教",
        "historicRoots": "时辰礼仪"
      },
      "catholic-evening-prayer": {
        "title": "天主教晚祷",
        "description": "借鉴晚祷，以圣咏、感恩与《尊主颂》结束一天。",
        "tradition": "天主教",
        "historicRoots": "时辰礼仪"
      },
      "catholic-night-prayer": {
        "title": "天主教夜祷",
        "description": "睡前的祷告节奏，带领您信靠神、寻求保护并安然交托。",
        "tradition": "天主教",
        "historicRoots": "寝前祷告"
      },
      "catholic-lectio-divina": {
        "title": "天主教圣言诵读",
        "description": "放慢阅读节奏，在经文中默想、祷告并停留。",
        "tradition": "天主教",
        "historicRoots": "圣言诵读"
      },
      "ignatian-daily-examen": {
        "title": "依纳爵每日省察",
        "description": "以感恩、回顾和悔改组成一天结束时的反思祷告。",
        "tradition": "天主教",
        "historicRoots": "依纳爵灵修"
      },
      "anglican-morning-prayer": {
        "title": "圣公会晨祷",
        "description": "以日课的形式，用圣咏和福音教导预备新的一天。",
        "tradition": "圣公会",
        "historicRoots": "公祷书"
      },
      "anglican-noonday-prayer": {
        "title": "圣公会午祷",
        "description": "在午间短暂祷告，以求助的诗篇和生命之粮的盼望重新得力。",
        "tradition": "圣公会",
        "historicRoots": "每日祷告"
      },
      "anglican-evening-prayer": {
        "title": "圣公会晚祷",
        "description": "经典的晚祷顺序，包含晚间诗篇和马利亚的颂歌。",
        "tradition": "圣公会",
        "historicRoots": "公祷书"
      },
      "anglican-compline": {
        "title": "圣公会寝前祷告",
        "description": "以认罪、信靠和安息组成温柔的夜间日课。",
        "tradition": "圣公会",
        "historicRoots": "公祷书"
      },
      "orthodox-morning-rule": {
        "title": "东正教晨祷规则",
        "description": "清晨按次序祷告，以悔改、怜悯和耶稣所教的祷告为核心。",
        "tradition": "东正教",
        "historicRoots": "晨祷"
      },
      "orthodox-sixth-hour": {
        "title": "东正教第六时辰祷告",
        "description": "在午间依照日课停下来祷告，记念基督受难并保持警醒。",
        "tradition": "东正教",
        "historicRoots": "定时祷告"
      },
      "orthodox-vespers": {
        "title": "东正教晚祷",
        "description": "在傍晚献上祷告，赞美创造，以灯光中的圣咏聆听圣言。",
        "tradition": "东正教",
        "historicRoots": "每日晚祷"
      },
      "orthodox-small-compline": {
        "title": "东正教小寝前祷告",
        "description": "安静的夜间日课，在睡前最后的祷告中寻求怜悯和庇护。",
        "tradition": "东正教",
        "historicRoots": "寝前祷告"
      },
      "benedictine-sacred-reading": {
        "title": "本笃会圣言阅读",
        "description": "受固定诵读时辰启发，以安静专注的阅读为核心。",
        "tradition": "本笃会",
        "historicRoots": "圣本笃会规"
      },
      "benedictine-psalm-and-work": {
        "title": "本笃会圣咏与劳作",
        "description": "让祷告、工作和安静的顺服贯穿一天的实用次序。",
        "tradition": "本笃会",
        "historicRoots": "祈祷与劳作"
      },
      "taize-evening-prayer": {
        "title": "泰泽晚祷",
        "description": "以诗篇、读经和安静构成歌唱与静默交织的夜晚。",
        "tradition": "泰泽",
        "historicRoots": "泰泽共同祷告"
      },
      "lutheran-morning-devotion": {
        "title": "路德宗晨间灵修",
        "description": "适合家庭的清晨灵修，以经文、背诵和恒常祷告开启一天。",
        "tradition": "路德宗",
        "historicRoots": "家庭每日灵修"
      },
      "lutheran-close-of-day": {
        "title": "路德宗晚间灵修",
        "description": "简朴的夜间灵修，在认罪中寻求基督的庇护和保守。",
        "tradition": "路德宗",
        "historicRoots": "家庭每日灵修"
      },
      "puritan-family-worship": {
        "title": "清教徒家庭敬拜",
        "description": "以家庭为中心的夜晚，在祷告、赞美、经文和敬虔的交谈中度过。",
        "tradition": "清教徒",
        "historicRoots": "家庭敬拜指南"
      }
    },
    "companions": {
      "mat-devotionals": {
        "title": "灵修",
        "description": "紧扣耶稣教导的简短默想。"
      },
      "mat-devotional-beatitudes": {
        "title": "在颠覆世俗价值的国度中蒙福",
        "summary": "默想八福，留意耶稣如何重塑我们的渴望。",
        "actionLabel": "阅读本章"
      },
      "mat-plans": {
        "title": "计划",
        "description": "有目的地通读马太福音的简明路径。"
      },
      "mat-plan-kingdom-rhythm": {
        "title": "马太福音中的天国节奏",
        "summary": "七天走过身份、祷告、怜悯、使命和复活的盼望。",
        "actionLabel": "打开第一章"
      },
      "jhn-playlists": {
        "title": "播放列表",
        "description": "精心编排的聆听路径，走过约翰福音的主要段落。"
      },
      "jhn-playlist-signs": {
        "title": "耶稣的神迹",
        "summary": "沿着显明基督身份的神迹，跟随引导阅读。",
        "actionLabel": "开始播放列表"
      },
      "gal-devotionals": {
        "title": "灵修",
        "description": "带着祷告阅读加拉太书的简短默想。"
      },
      "gal-devotional-no-longer-slaves": {
        "title": "不再为奴",
        "summary": "静思成为神的儿女、承受产业与天父的接纳。",
        "actionLabel": "阅读本章"
      },
      "gal-plans": {
        "title": "计划",
        "description": "以简短有序的路径通读这封书信。"
      },
      "gal-plan-six-days": {
        "title": "每日一章：加拉太书",
        "summary": "每天读一章，不急不躁地理解保罗的论述。",
        "actionLabel": "打开第一章"
      },
      "psa-playlists": {
        "title": "播放列表",
        "description": "适合不同心境的祷告路径。"
      },
      "psa-playlist-morning": {
        "title": "清晨的避难所",
        "summary": "以信靠的诗篇开启一天的简短路径。",
        "actionLabel": "开始播放列表"
      },
      "psa-playlist-repentance": {
        "title": "悔改的诗篇",
        "summary": "以诚实、认罪与更新来祷告。",
        "actionLabel": "开始播放列表"
      },
      "psa-devotionals": {
        "title": "灵修",
        "description": "放慢脚步，以诗篇祷告的安静默想。"
      },
      "psa-devotional-shepherd": {
        "title": "使人复苏的牧者",
        "summary": "在诗篇23篇中停留，直到它成为祷告，而不只是口号。",
        "actionLabel": "阅读诗篇"
      }
    },
    "translationDescriptions": {
      "bsb": "公版《庇哩亚标准圣经》文本，附可直接播放的 CC0 章节音频",
      "web": "公版英国版文本与章节音频，来自 EveryBible 资料库",
      "kjv": "1611年钦定《英王詹姆斯译本》",
      "asv": "1901年出版的《钦定版》修订本，文本已预装",
      "bbe": "使用有限的词汇，便于阅读",
      "sparv1909": "经典西班牙语圣经，公版文本",
      "hincv": "印地语当代译本，属公版",
      "npiulb": "开放直译圣经，属公版"
    },
    "audioUnavailableTranslation": "此译本暂无音频。",
    "audioUnavailableChapter": "此章节暂无音频。",
    "audioPlayFailed": "无法播放此音频，请重试。",
    nativePermissions: {
      "NSCameraUsageDescription": "如果您选择拍摄个人资料照片，请允许 Every Bible 使用相机。",
      "NSMicrophoneUsageDescription": "允许 Every Bible 录制语音回复，用于章节反馈。",
      "NSPhotoLibraryUsageDescription": "允许 Every Bible 访问您的照片图库，以便选择个人资料照片。",
      "NSFaceIDUsageDescription": "允许 Every Bible 使用 Face ID 保护您保存的登录信息。"
    }
  },
  translatorQueue: {
    title: "反馈待办",
    subtitle: "有反馈待审核的章节",
    empty: "全部处理完毕，暂无待审核的反馈。",
    pendingCount: "{{count}} 章待审核",
    chapterCounts: "{{down}} 项待修正 · {{up}} 项待确认",
    openLabel: "打开 {{reference}}",
  },
  myFeedback: {
    title: "我的反馈",
    settingsRow: "我的反馈",
    settingsRowSummary: "查看翻译团队如何处理您的意见",
    subtitle: "翻译团队对您的章节反馈的处理情况",
    empty: "您还没有提交过章节反馈。",
    signInRequired: "登录以查看您提交的反馈。",
    audioLabel: "已附上语音留言",
    statusReceived: "已收到",
    statusFixed: "已修正",
    statusNoChange: "已审核，无需修改",
  },
  "common": {
    "cancel": "取消",
    "ok": "好的",
    "done": "完成",
    "error": "错误",
    "retry": "重试",
    "loading": "加载中...",
    "continue": "继续",
    "save": "保存",
    "delete": "删除",
    "edit": "编辑",
    "back": "返回",
    "next": "下一个",
    "previous": "上一个",
    "search": "搜索",
    "settings": "设置",
    "comingSoon": "即将推出",
    "available": "可用的",
    "notSet": "未设置",
    "unexpectedError": "很抱歉，但意外发生了。请再试一次。",
    "somethingWentWrong": "出了点问题",
    "tryAgain": "再试一次",
    "shareMessage": "查看EveryBible！",
    "all": "全部"
  },
  "tabs": {
    "home": "家",
    "bible": "圣经",
    "harvest": "收成",
    "gather": "聚集",
    "more": "更多的",
    "plans": "计划"
  },
  "home": {
    "allTime": "全部时间",
    "ledgerChapters": "章",
    "ledgerChaptersCaption": "已读或已听",
    "ledgerBooksFinished": "读完的书卷",
    "ledgerBooksFinishedCaption": "从头到尾",
    "ledgerNextUp": "接下来 · {{reference}}，共 {{total}} 章",
    "ledgerNoBooksFinished": "暂无",
    "ledgerNoChapters": "还没有章节",
    "ledgerPeriodLabel": "阅读统计时间段",
    "ledgerSince_one": "自 {{date}} · {{count}} 章",
    "ledgerSince_other": "自 {{date}} · {{count}} 章",
    "ledgerThisMonth": "{{month}} · {{total}} 天中的 {{active}} 天",
    "ledgerThisWeek": "本周 · {{total}} 天中的 {{active}} 天",
    "streakUnitLabel": "天连续",
    "dayEyebrow": "天数",
    "lessonsProgress": "{{completed}} / {{total}} 节课程",
    "nextLesson": "下一节 · {{title}}",
    "readPassage": "阅读{{passage}}",
    "goodMorning": "早上好",
    "goodAfternoon": "下午好",
    "goodEvening": "晚上好",
    "greetingWithName": "{{greeting}}, {{name}}",
    "guestName": "朋友",
    "welcome": "欢迎来到Every Bible",
    "beginToday": "今天开始",
    "todaysScripture": "今日经文",
    "verseOfTheDay": "每日经文",
    "sectionOfTheDay": "当日部分",
    "sectionOfTheDayBody": "即使尚未添加书面文本，今天的选择也可以作为本章的音频提供。",
    "verseAudioBody": "今天的经文目前以音频形式提供，因此您甚至可以在添加书面文本之前收听。",
    "playSectionOfTheDay": "当日播放部分",
    "playVerseOfTheDay": "播放每日经文",
    "continueReading": "继续阅读",
    "sharePrompt": "分享光亮。今天鼓励一个人。",
    "plan": "计划",
    "minutesLeft": "还剩 {{count}} 分钟",
    "percentComplete": "{{percent}}%",
    "fieldLabel": "字段 {{number}}",
    "notificationSettings": "通知设置",
    "chaptersRead": "已读章节",
    "today": "今天",
    "week": "星期",
    "month": "月",
    "year": "年",
    "defaultVerse": "开始阅读或聆听来发现今天的经文。",
    "defaultReference": "打开圣经"
  },
  "onboarding": {
    "allNations": "所有国家",
    "continueWithNation": "选择{{name}}并继续",
    "countryLanguageCount_one": "{{count}} 种语言",
    "countryLanguageCount_other": "{{count}} 种语言",
    "countrySearchPlaceholderCount": "在 {{total}} 个国家中搜索",
    "noNationsFound": "暂无匹配的国家",
    "noNationsFoundBody": "请换一种拼写，或使用英文或当地文字搜索。",
    "searchAboveHint": "没有找到？请在上方搜索",
    "searchResults": "搜索结果",
    "stepEyebrow": "第 {{step}} 步，共 {{total}} 步",
    "suggestedBadge": "推荐",
    "suggestedFromDevice": "根据你的设备推荐",
    "stepCountry": "{{count}}的步骤1",
    "stepLanguage": "{{count}}的步骤2",
    "stepPrivacy": "第 3 步（共 3 步）",
    "interfaceLanguageTitle": "选择您的界面语言",
    "interfaceLanguageBody": "首先选择菜单、按钮和设置的语言。您可以稍后再更改。",
    "interfaceLanguageSearchPlaceholder": "搜索界面语言",
    "availableInterfaceLanguages": "可用的界面语言",
    "countryTitle": "选择你的国家",
    "countryBody": "我们首先使用您所在的国家/地区推荐附近的语言，同时仍然允许您进行全球搜索。",
    "countrySearchPlaceholder": "寻找你的国家",
    "languageTitle": "选择您的圣经语言",
    "languageBody": "我们推荐 {{country}} 中使用的圣经语言，但您仍然可以跨境搜索任何语言。",
    "languageSearchPlaceholder": "搜索或输入您的语言",
    "recommendedLanguages": "{{country}} 中推荐的圣经语言",
    "moreLanguages": "其他匹配语言",
    "noLanguagesFound": "尚无语言匹配",
    "noLanguagesFoundBody": "尝试其他拼写或用英语或本地文字搜索。模糊搜索已启用。",
    "recommendedBadge": "受到推崇的",
    "interfaceAvailable": "接口可用",
    "privacyTitle": "选择您的隐私风格",
    "privacyBody": "选择该设备的图标样式。每当您离开应用程序时，谨慎模式还会添加本地密码锁。",
    "standardIconTitle": "标准图标",
    "standardIconBody": "使用普通的Every Bible图标并直接打开应用程序。",
    "discreetIconTitle": "谨慎的图标",
    "discreetIconBody": "在此设备上使用低调图标，并使用计算器式代码保护访问。",
    "pinTitle": "创建您的安全代码",
    "pinBody": "使用数字和符号（如 +、-、x 或 /）选择 4 到 6 个字符。每次返回应用程序时，您都需要输入此代码。",
    "pinPlaceholder": "输入安全代码",
    "pinConfirmPlaceholder": "确认安全代码",
    "pinLegend": "允许的字符：0-9、+、-、x 和 /",
    "finish": "完成设置"
  },
  "bible": {
    translatorReviewAudioError: "无法播放此音频，请重试。",
    chapterFeedbackSuccessTitle: "反馈已发送",
    readerFontsAndSettings: "字体与设置",
    "title": "圣经",
    "oldTestament": "旧约",
    "fontsAndSettings": "字体与设置",
    "allSettings": "所有设置",
    "backToPlayer": "返回播放器",
    "packMissingRecovering": "此译本的文件丢失了。正在恢复中——请稍后再试。",
    "relatedVerses": "相关经文",
    "newTestament": "新约",
    "fullBible": "完整圣经",
    "byBook": "按卷",
    "chapters": "章节",
    "chapter": "章",
    "verse": "节经文",
    "selectTranslation": "选择翻译",
    "manageAudio": "管理音频",
    "audioDownloads": "音频下载",
    "downloadBibleAudio": "下载整本圣经音频",
    "audioSavedOffline": "已保存以供离线收听",
    "audioDownloadFailed": "我们现在无法下载此音频。",
    "available": "可用的",
    "translationComingSoon": "{{name}} 将在未来的更新中提供下载。",
    "failedToLoad": "我们现在无法加载本章。",
    "searchUnavailable": "此翻译尚不支持全文搜索。",
    "noVersesAvailable": "{{book}} {{chapter}} 还没有文字。",
    "fullBibleComingSoon": "本章目前没有文本版本。如果音频可用，Every Bible 将打开聆听体验。",
    "notAvailableYet": "尚未提供",
    "bookComingSoon": "{{book}}在此译本中尚未提供文本或音频。",
    "audioOnlyTitle": "以音频为主的章节",
    "audioOnlyBody": "{{translation}} 目前可作为本章的音频提供，而书面文本仍在准备中。",
    "verseCount_one": "{{count}} 节经文",
    "verseCount_other": "{{count}} 节经文",
    "listen": "收听",
    "read": "阅读",
    "removeFromFavorites": "从收藏夹中删除",
    "addToFavorites": "添加到收藏夹",
    "addToSavedPlaylist": "添加到已保存的播放列表",
    "addToQueue": "添加到队列",
    "downloadBookAudio": "下载书籍音频",
    "shareChapterReference": "分享章节参考",
    "shareChapterAudio": "分享章节音频",
    "shareAudioPortion": "分享音频部分",
    "shareVerseImage": "图像",
    "chooseVerseImageBackground": "选择背景",
    "books": {
      "GEN": "创世记",
      "EXO": "出埃及记",
      "LEV": "利未记",
      "NUM": "民数记",
      "DEU": "申命记",
      "JOS": "约书亚记",
      "JDG": "士师记",
      "RUT": "路得记",
      "1SA": "撒母耳记上",
      "2SA": "撒母耳记下",
      "1KI": "列王纪上",
      "2KI": "列王纪下",
      "1CH": "历代志上",
      "2CH": "历代志下",
      "EZR": "以斯拉记",
      "NEH": "尼希米记",
      "EST": "以斯帖记",
      "JOB": "约伯记",
      "PSA": "诗篇",
      "PRO": "箴言",
      "ECC": "传道书",
      "SNG": "雅歌",
      "ISA": "以赛亚书",
      "JER": "耶利米书",
      "LAM": "耶利米哀歌",
      "EZK": "以西结书",
      "DAN": "但以理书",
      "HOS": "何西阿书",
      "JOL": "约珥书",
      "AMO": "阿摩司书",
      "OBA": "俄巴底亚书",
      "JON": "约拿书",
      "MIC": "弥迦书",
      "NAM": "那鸿书",
      "HAB": "哈巴谷书",
      "ZEP": "西番雅书",
      "HAG": "哈该书",
      "ZEC": "撒迦利亚书",
      "MAL": "玛拉基书",
      "MAT": "马太福音",
      "MRK": "马可福音",
      "LUK": "路加福音",
      "JHN": "约翰福音",
      "ACT": "使徒行传",
      "ROM": "罗马书",
      "1CO": "哥林多前书",
      "2CO": "哥林多后书",
      "GAL": "加拉太书",
      "EPH": "以弗所书",
      "PHP": "腓立比书",
      "COL": "歌罗西书",
      "1TH": "帖撒罗尼迦前书",
      "2TH": "帖撒罗尼迦后书",
      "1TI": "提摩太前书",
      "2TI": "提摩太后书",
      "TIT": "提多书",
      "PHM": "腓利门书",
      "HEB": "希伯来书",
      "JAS": "雅各书",
      "1PE": "彼得前书",
      "2PE": "彼得后书",
      "1JN": "约翰一书",
      "2JN": "约翰二书",
      "3JN": "约翰三书",
      "JUD": "犹大书",
      "REV": "启示录"
    },
    "chapterFeedback": "章节反馈",
    "chapterFeedbackTitle": "审核这段经文",
    "chapterFeedbackBody": "请告诉我们本章是否有帮助或者哪些方面可以改进。评论是可选的。",
    "chapterFeedbackThumbsUp": "有帮助",
    "chapterFeedbackThumbsDown": "需要工作",
    "chapterFeedbackPlaceholder": "关于本章的可选评论",
    "chapterFeedbackSelectionHint": "点击“大拇指向上”或“大拇指向下”以添加可选注释。",
    "chapterFeedbackSubmit": "发送反馈",
    "chapterFeedbackAudioIdle": "也可以录音回复",
    "chapterFeedbackAudioRecording": "录音中 {{duration}}",
    "chapterFeedbackAudioReady": "语音回复已就绪：{{duration}}",
    "chapterFeedbackAudioLimit": "M4A 格式，最长 1 分钟",
    "chapterFeedbackAudioRecord": "录音",
    "chapterFeedbackAudioStop": "停止",
    "chapterFeedbackAudioPreview": "试听语音回复",
    "chapterFeedbackAudioRerecord": "重新录制语音回复",
    "chapterFeedbackAudioUploading": "正在上传音频…",
    "chapterFeedbackAudioPermissionDenied": "麦克风权限已关闭，您仍可输入文字回复。",
    "chapterFeedbackAudioPermissionHelp": "请在系统设置中开启麦克风权限，以提交语音反馈。",
    "chapterFeedbackAudioStartError": "无法开始录音，您仍可输入文字回复。",
    "chapterFeedbackAudioStopError": "无法完成此次录音，请重试。",
    "chapterFeedbackAudioRecordingMissing": "无法保存录音，请重试。",
    "chapterFeedbackAudioUploadError": "音频上传失败。录音仍然保留，请重新提交。",
    "chapterFeedbackSuccess": "感谢您的反馈。您提交的内容已收到，我们的翻译团队将尽快进行审核。",
    "chapterFeedbackSignInRequired": "请先登录，然后再发送章节反馈。",
    "translatorReviewTitle": "准确性审核",
    "translatorReviewSummary": "待处理：{{pending}} 项，共 {{count}} 项",
    "translatorReviewSummaryComplete": "所有准确性审核均已处理",
    "translatorReviewLoading": "正在加载反馈…",
    "translatorReviewEmpty": "此章节尚未收到反馈。",
    "translatorReviewUnknownUser": "未知贡献者",
    "translatorReviewUnread": "待决定",
    "translatorReviewSubmittedAt": "提交于 {{date}}",
    "translatorReviewSubmittedBy": "由 {{name}} 提交",
    "translatorReviewNoComment": "没有文字反馈。",
    "translatorReviewFixed": "已修正",
    "translatorReviewConfirmedAccurate": "已确认准确",
    "translatorReviewReviewed": "无需修改",
    "translatorReviewConfirmAccurate": "确认准确",
    "translatorReviewMarkFixed": "标记为已修正",
    "translatorReviewNoActionNeeded": "无需处理",
    "translatorReviewReopen": "重新开启",
    "translatorReviewListened": "已听取",
    "translatorReviewListen": "收听",
    "translatorReviewPause": "暂停",
    "nextChapterHint": "进入下一章",
    "openBookAndChapterPickerHint": "打开书籍和章节选择器",
    "openTranslationOptionsHint": "打开翻译选项",
    "returnToPlanHint": "返回计划详细信息屏幕"
  },
  "audio": {
    "playbackSpeed": "播放速度",
    "sleepTimer": "睡眠定时器",
    "nowPlaying": "正在播放",
    "readyToResume": "准备恢复",
    "repeatChapter": "重复章节",
    "repeatBook": "重复书",
    "repeatOff": "重复关闭",
    "showText": "文字",
    "showTextHint": "打开当前播放章节的圣经文本",
    "musicAndSounds": "音乐与声音",
    "chooseBackgroundLayer": "选择一个内置的背景音层，以便离线聆听经文。",
    "previousChapter": "上一章",
    "nextChapter": "下一章",
  },
  "privacy": {
    "lockTitle": "输入您的安全代码",
    "lockBody": "该设备处于谨慎模式。解锁 Every Bible 以继续。",
    "pinPlaceholder": "点按您的代码",
    "pinLengthHint": "使用 4 到 6 个字符的代码。",
    "unlock": "解锁圣经",
    "clearKey": "清除",
    "deleteKey": "删除",
    "pinTooShort": "您的安全代码必须至少包含 4 个字符。",
    "pinTooLong": "您的安全代码不能超过 6 个字符。",
    "pinInvalidCharacters": "仅允许使用数字和计算器符号。",
    "pinMismatch": "安全代码尚不匹配。",
    "incorrectPin": "该安全代码不正确。"
  },
  "settings": {
    "title": "设置",
    "reading": "阅读",
    "hidePlayButtonFromReadingTab": "在阅读标签页中隐藏播放按钮",
    "language": "界面语言",
    "nationAndLanguage": "民族和圣经语言",
    "fontSize": "字体大小",
    "fontSizeSmall": "小的",
    "fontSizeMedium": "中等的",
    "fontSizeLarge": "大的",
    "darkMode": "深色模式",
    "themeMode": "主题",
    "themeDark": "黑暗的",
    "themeLight": "光",
    "themeLowLight": "低光",
    "themeParchment": "羊皮纸",
    "themeMidnight": "午夜",
    "appearance": "外貌",
    "appearanceBody": "选择整个应用程序中使用的强调调色板。",
    "appearanceEmberTitle": "余烬",
    "appearanceEmberBody": "温暖、接地气，最接近现在的造型。",
    "appearanceSapphireTitle": "蓝宝石",
    "appearanceSapphireBody": "平静的蓝色，对比强烈，给人清爽的感觉。",
    "appearanceTealTitle": "青色",
    "appearanceTealBody": "平衡而清晰，但又不会感觉太酷。",
    "appearanceOliveTitle": "橄榄",
    "appearanceOliveBody": "朴实而安静，带有柔和的书卷气。",
    "notifications": "通知",
    "dailyReminder": "每日提醒",
    "reminderTime": "提醒时间",
    "notSet": "未设置",
    "data": "数据",
    "downloadOffline": "离线下载",
    "downloadForOffline": "离线下载",
    "clearCache": "清除缓存",
    "clearCacheConfirm": "这将清除缓存的数据。您的阅读进度将被保留。",
    "clear": "清除",
    "cacheClearedSuccess": "缓存清除成功",
    "cacheClearError": "清除缓存时出错",
    "setReminderTime": "设置提醒时间",
    "setTime": "设定时间",
    "permissionRequired": "需要许可",
    "enableNotifications": "请在您的设备设置中启用通知以接收每日提醒。",
    "enableNotificationsMessage": "请在您的设备设置中启用通知以接收每日提醒。",
    "notificationTitle": "每日圣经阅读",
    "notificationBody": "每天读圣经的时间到了！",
    "selectLanguage": "选择界面语言",
    "deleteAccount": "删除帐户",
    "deleteAccountWarning": "这将永久删除您的帐户和所有相关数据，包括阅读进度、偏好设置和同步内容。此操作无法撤消。",
    "accountDeleted": "帐户已删除",
    "accountDeletedMessage": "您的帐户和所有数据已被删除。",
    "deleteAccountError": "删除帐户失败。请再试一次。",
    "notSignedIn": "您必须登录才能删除您的帐户。",
    "diagnostics": {
      "title": "诊断",
      "description": "此设备上记录的近期错误和崩溃。请将其分享给支持团队，帮助我们解决问题。",
      "emptyTitle": "未记录任何问题",
      "emptyBody": "如果发生错误和崩溃，将显示在此处。",
      "badgeFatal": "崩溃",
      "badgeError": "错误",
      "clear": "清除",
      "clearTitle": "清除诊断",
      "clearConfirm": "从此设备中移除所有已记录的错误和崩溃日志吗？",
      "exportHeader": "EveryBible 诊断报告",
      "exportError": "无法分享诊断报告。",
    },
    "delete": "删除",
    "chapterFeedbackIdentity": "反馈身份",
    "chapterFeedbackIdentityTitle": "这个反馈应该来自谁？",
    "chapterFeedbackIdentityBody": "添加您的姓名和角色，以便在 Google 表格中正确标注每个章节的评分。",
    "chapterFeedbackIdentityRole": "在项目中的角色",
    "chapterFeedbackIdentityRolePlaceholder": "教会领袖、语言演讲者、社区长老",
    "chapterFeedbackIdentityRequired": "请输入您的姓名和角色。",
    "chapterFeedbackIdentitySummaryOff": "尚未保存反馈身份",
    "chapterFeedback": "章节反馈",
    "chapterFeedbackSummaryOn": "可选章节反馈已开启",
    "chapterFeedbackSummaryOff": "可选章节反馈已关闭",
    "translatorAccess": "译者权限",
    "translatorAccessTitle": "译者权限",
    "translatorAccessBody": "输入译者密码，即可在此设备上审核章节反馈。",
    "translatorAccessPlaceholder": "密码",
    "translatorAccessUnlock": "解锁",
    "translatorAccessIncorrect": "译者密码不正确。",
    "translatorAccessEnabled": "已启用译者审核模式",
    "translatorAccessEnabledBody": "章节页面现在将显示译者反馈审核工具。",
    "translatorAccessSummaryOn": "译者审核工具已启用",
    "translatorAccessSummaryOff": "输入密码以启用审核工具",
  },
  "notifications": {
    "channelDailyReminder": "每日读书提醒",
    "channelGroupAlerts": "小组会议提醒",
    "groupSessionTitle": "小组会议已完成",
    "groupSessionBody": "会话记录于 {{groupName}}"
  },
  "more": {
    "footerVersion": "EveryBible {{version}} · Every Language",
    "groupAccount": "账户",
    "groupApp": "应用",
    "groupContent": "内容",
    "highlightsAndNotes": "亮点与笔记",
    "reminderValue": "{{time}} 提醒",
    "streakValue_one": "连续 {{count}} 天",
    "streakValue_other": "连续 {{count}} 天",
    "translations": "译本",
    "translationsValue": "{{abbreviation}} · {{count}} 个已离线",
    "sync": {
      "relativeDays": "{{days}} 天前",
      "relativeHours": "{{hours}} 小时前",
      "relativeMinutes": "{{minutes}} 分钟前",
      "relativeNow": "刚刚",
      "signInToSync": "登入以同步",
      "source": "来源：本设备",
      "sourceSynced": "来源：本设备 · {{relative}}已同步",
      "synced": "已同步",
      "syncedAgo": "{{relative}}已同步",
    },
    "title": "更多的",
    "guestUser": "访客用户",
    "signInToSync": "登录以同步您的进度",
    "signInOrCreate": "登录或创建帐户",
    "readingActivity": "阅读活动",
    "signOut": "登出",
    "signOutConfirm": "您确定要退出吗？您本地的阅读进度将被保留。",
    "profile": "轮廓",
    "settings": "设置",
    "about": "关于",
    "syncYourProgress": "同步您的进度"
  },
  "auth": {
    "accountEyebrow": "账户",
    "createAnAccount": "创建一个账户",
    "hidePassword": "隐藏密码",
    "newHere": "初次使用？",
    "orWithEmail": "或使用电子邮件",
    "showPassword": "显示密码",
    "tagline": "永久免费 · 无广告 · 各种语言",
    "welcomeBack": "欢迎回来",
    "signInToSync": "登录以同步您的阅读进度",
    "signInSubtitle": "登录以跨设备同步您的阅读进度",
    "signUpSubtitle": "创建一个帐户来同步您的进度",
    "email": "电子邮件",
    "emailPlaceholder": "you@example.com",
    "emailRequired": "电子邮件为必填项",
    "emailInvalid": "请输入有效的电子邮件",
    "emailRequiredForReset": "请先输入您的电子邮件地址",
    "password": "密码",
    "passwordPlaceholder": "输入您的密码",
    "passwordRequired": "需要密码",
    "passwordMinLength": "密码必须至少为 6 个字符",
    "passwordHint": "至少 6 个字符",
    "confirmPassword": "确认密码",
    "confirmPasswordPlaceholder": "重新输入您的密码",
    "passwordsDoNotMatch": "密码不匹配",
    "name": "姓名",
    "namePlaceholder": "你的名字",
    "nameRequired": "姓名为必填项",
    "signIn": "登入",
    "signUp": "报名",
    "createAccount": "创建账户",
    "forgotPassword": "忘记密码？",
    "noAccount": "没有帐户？",
    "haveAccount": "已经有帐户？",
    "alreadyHaveAccount": "已经有帐户？",
    "or": "或者",
    "continueWithApple": "继续 Apple",
    "continueWithGoogle": "继续 Google",
    "signInFailed": "登录失败",
    "signUpFailed": "注册失败",
    "checkCredentials": "请检查您的凭据并重试",
    "signInCancelled": "登录已取消",
    "appleSignInFailed": "Apple 登录失败",
    "googleSignInFailed": "Google 登录失败",
    "signInAlreadyInProgress": "登录已在进行中",
    "providerUnavailable": "此登录方式目前不可用",
    "serviceUnavailable": "EveryBible 无法连接到服务器。请稍后再试。",
    "backendNotConfigured": "此版本尚未配置 EveryBible。",
    "somethingWentWrong": "出了点问题。请再试一次。",
    "checkYourEmail": "检查您的电子邮件",
    "resetLinkSent": "我们向您发送了密码重置链接",
    "resetEmailError": "发送重置邮件失败",
    "accountCreated": "帐户已创建",
    "verifyEmailMessage": "请检查您的电子邮件以验证您的帐户",
    "resetPasswordTitle": "设置新密码",
    "resetPasswordSubtitle": "为您的账户选择一个新密码",
    "newPassword": "新密码",
    "newPasswordPlaceholder": "输入新密码",
    "confirmNewPassword": "确认新密码",
    "resetPasswordSubmit": "更新密码",
    "resetPasswordSuccess": "您的密码已更新",
    "resetPasswordError": "无法更新您的密码",
    "resetLinkConfirmTitle": "设置新密码？",
    "resetLinkConfirmBody": "此链接将为 {{email}} 设置新密码。要继续吗？",
    "resetLinkDifferentAccount": "此重置链接属于其他账号。请先退出当前账号，然后重新打开该链接。",
    "resetPasswordInvalidSession": "此重置链接无效或已过期。请重新申请一个。",
  },
  "profile": {
    "streak": "连续天数",
    "readingActivity": "阅读活动",
    "readingActivitySubtitle": "一目了然地查看您的阅读天数和连读次数。",
    "readingDays": "读书日",
    "chaptersRead": "已读章节",
    "chapterRead": "已读章节",
    "selectedDay": "选定日期",
    "tapDayHint": "点击标记的日期即可查看详细信息。",
    "noReadingActivityTitle": "还没有阅读活动",
    "noReadingActivityBody": "打开一章开始填写您的阅读日历。",
    "firstReadAt": "首次阅读时间为 {{time}}",
    "lastReadAt": "上次阅读时间：{{time}}",
    "changeAvatar": "更改照片",
    "uploadingAvatar": "正在上传...",
    "avatarUpdated": "照片已更新",
    "avatarUpdateFailed": "无法更新照片"
  },
  "fields": {
    "entryTitle": "入口",
    "entrySubtitle": "空场",
    "entryDesc": "通过祈祷寻找和平的人并准备好心灵",
    "gospelTitle": "福音",
    "gospelSubtitle": "播种田",
    "gospelDesc": "以简单、可重复的方式分享好消息",
    "discipleshipTitle": "门徒训练",
    "discipleshipSubtitle": "成长领域",
    "discipleshipDesc": "基督给新信徒的七条命令",
    "churchTitle": "教会",
    "churchSubtitle": "收获场",
    "churchDesc": "像简单的教堂一样聚集在一起",
    "multiplicationTitle": "乘",
    "multiplicationSubtitle": "复制",
    "multiplicationDesc": "培养和派遣领袖（提摩太后书 2:2）"
  },
  "harvest": {
    "title": "收成",
    "subtitle": "在你的信仰之旅中成长",
    "heroTitle": "每个国家，每个语言",
    "heroSubtitle": "使门徒倍增 使门徒倍增",
    "progressLabel": "你的旅程",
    "fiveFieldsTitle": "五个领域",
    "fiveFieldsSubtitle": "乘法之路",
    "training": "收获训练",
    "journey1Field1Goal": "1 场 1 目标之旅",
    "journeyDescription": "以集中的方式帮助未触及的人群",
    "fourFieldsJourney": "四田之旅",
    "fourFieldsDescription": "培养门徒的可复制途径",
    "lessonsCompleted": "{{total}} 课程的 {{completed}}",
    "fieldsComplete": "{{count}} 5个完整",
    "startJourney": "开始旅程",
    "continueJourney": "继续旅程",
    "start": "开始旅程",
    "continue": "继续旅程",
    "begin": "开始",
    "active": "积极的",
    "goodNews": "好消息",
    "makingDisciples": "培养门徒",
    "multiplication": "乘法",
    "migrationNotice": "我们将培训简化为 3 个重点领域",
    "fieldComplete": "现场完成",
    "nextField": "下一个字段",
    "currentField": "当前领域",
    "completedFields": "已完成的字段",
    "trainingOverviewTitle": "收获训练",
    "trainingOverviewBody": "为日常服从、实践和倍增而设计的结构化门徒训练路径。",
    "browseFields": "浏览字段",
    "nextStep": "下一步",
    "nextLesson": "下一课",
    "nextLessonDescription": "从上次停下的地方继续，继续积蓄动力。",
    "exploreField": "探索领域",
    "allLessonsComplete": "所有课程已完成",
    "allLessonsCompleteBody": "您已完成当前的训练路径。重新审视任何领域，继续练习，并教别人。",
    "fieldProgress": "{{progress}} 完成%",
    "keyVerse": "核心经文",
    "practiceActivity": "实践活动",
    "discussionLabel": "讨论",
    "prayerLabel": "祷告",
    "discussionQuestions": "讨论问题",
    "keyTakeaway": "要点",
    "shareTakeaway": "分享这个",
    "weeklyPractice": "本周练习",
    "practiceDone": "我做到了",
    "practiceDoneComplete": "我做到了！",
    "practiceDoneHint": "完成练习后进行标记",
    "taughtThis": "我教过这个",
    "taughtThisComplete": "这个是我教的！",
    "taughtThisHint": "标记您与某人分享的时间",
    "encouragement222": "惊人的！通过传递这一点，你就活出了《提摩太后书》2:2。",
    "readInContext": "结合上下文阅读",
    "groupPreviewBadge": "生产强化",
    "groupPreviewTitle": "学习小组",
    "groupPreviewBody": "正在重建小组学习，以实现跨设备的安全同步。现有本地组将保留在此设备上，直到同步部署准备就绪。",
    "groupPreviewCta": "查看群组状态",
    "localGroupsTitle": "该设备上的本地群组",
    "localGroupsDescription": "这些组仅保存在本地，尚未同步。",
    "localOnly": "仅限本地",
    "noLocalGroups": "此设备上尚未保存任何本地组。",
    "syncedGroupsTitle": "同步群组",
    "syncedGroupsDescription": "当为此版本启用安全同步时，这些组来自您的登录帐户。",
    "syncedGroupsSignin": "登录以查看与您的帐户绑定的同步群组。",
    "noSyncedGroups": "此帐户尚无可用的同步群组。",
    "syncedLabel": "已同步",
    "loadingSyncedGroups": "正在检查链接到此帐户的同步群组...",
    "groupSyncLoadError": "目前无法刷新已同步的组。",
    "groupSyncPending": "此版本中尚未启用同步组。本地群组保留在此设备上。",
    "groupSyncSignin": "立即登录，以便在启用同步群组时您的帐户已准备就绪。",
    "groupSyncReady": "一旦推出开始，您的帐户就可以同步群组了。",
    "scriptureNavigationUnavailable": "培训课程的圣经导航即将推出。",
    "lessonNotFound": "找不到课程",
    "fieldNotFound": "未找到字段",
    "goBack": "回去",
    "completeAndContinue": "完成并继续",
    "completeLesson": "完整课程",
    "studies": "研究",
    "chapters": "章节",
    "nowReading": "现在在圣经选项卡中打开",
    "openAndPlay": "打开并播放",
    "chapterStudiesSubtitle": "专为全文阅读和聆听圣经而设计的专题章节研究。",
    "chapterStudiesBody": "每项研究都会打开完整的章节，因此音频播放和阅读在跨设备的上下文中保持可靠。",
    "playChapterSubtitle": "打开并播放",
    "lessons": {
      "entry1": "祈求丰收",
      "entry2": "您的奥伊科斯地图",
      "entry3": "寻找和平之人",
      "entry4": "你的故事很重要",
      "gospel1": "简单的福音",
      "gospel2": "三圈",
      "gospel3": "回答问题",
      "gospel4": "邀请回应",
      "gospel5": "立即受洗",
      "disc1": "悔改并相信",
      "disc2": "受洗",
      "disc3": "祈祷",
      "disc4": "彼此相爱",
      "disc5": "一起掰面包",
      "disc6": "慷慨给予",
      "disc7": "使人作门徒",
      "church1": "什么是教会？",
      "church2": "简单的收集元素",
      "church3": "大家都参与",
      "church4": "任命领导人",
      "mult1": "乘法思维",
      "mult2": "辅导与教学",
      "mult3": "发送与支持",
      "mult4": "第四代的愿景"
    }
  },
  "about": {
    "title": "关于",
    "version": "版本{{version}}",
    "description": "Every Bible 是一种简单、美丽的阅读和学习神的话语的方式。",
    "bibleTranslation": "圣经翻译",
    "bereanBible": "庇哩亚标准圣经",
    "privacyPolicy": "隐私政策",
    "termsOfService": "服务条款",
    "resources": "资源",
    "madeWithLove": "用爱制造"
  },
  "loading": {
    "initializing": "正在初始化...",
    "loadingBible": "加载圣经数据...",
    "settingUp": "正在设置...",
    "ready": "准备好！",
    "errorLoading": "加载数据时出错"
  },
  "readingPlans": {
    "activeCount": "{{count}} 个进行中",
    "completedCount": "{{count}} 个已完成",
    "dailyRhythm": "每日节奏",
    "day": "天数",
    "dayChapterCount_one": "{{count}} 章",
    "dayChapterCount_other": "{{count}} 章",
    "daysCount": "{{count}} 天",
    "durationDays_one": "{{count}} 天",
    "durationDays_other": "{{count}} 天",
    "leavePlan": "退出计划",
    "leavePlanConfirmBody": "此计划的进度将从本设备中移除。",
    "ledger": "记录",
    "planOptions": "计划选项",
    "plansCount_one": "{{count}} 个计划",
    "plansCount_other": "{{count}} 个计划",
    "readMissedSummary": "已读 {{read}} · 错过 {{missed}}",
    "readSummary": "已读 {{read}}",
    "searchPlansCount": "在 {{count}} 个计划中搜索",
    "start": "开始",
    "tomorrow": "明天",
    "title": "阅读计划",
    "browsePlans": "浏览计划",
    "dailyRhythms": "每日节奏",
    "dailyReadings": "每日读经",
    "myPlans": "我的计划",
    "startPlan": "开始计划",
    "enrolled": "已注册",
    "dayOf": "第 {{current}} 天（共 {{total}} 天）",
    "markComplete": "标记完成",
    "completed": "已完成",
    "progress": "进度",
    "noPlans": "没有可用的阅读计划",
    "noActivePlans": "你还没有开始任何计划",
    "findPlans": "查找计划",
    "saved": "已保存",
    "completions": "{{count}} 次完成",
    "saveForLater": "保存以供稍后使用",
    "unsave": "未保存",
    "sample": "样本",
    "featuredPlan": "特色计划",
    "relatedPlans": "相关计划",
    "noSavedPlans": "尚未保存计划",
    "noCompletedPlans": "尚未完成计划",
    "todayTargetProgress": "今天的目标：{{completed}}/{{target}} 章",
    "listenTargetLabel": "今天的计划",
    "listenTargetProgress": "今天的目标：{{completed}}/{{target}} 章",
    "listenTargetComplete": "每日目标完成",
    "listenTargetRemainingOne": "今天还剩1章",
    "listenTargetRemainingOther": "今天还剩 {{count}} 章",
    "dayLabel": "第 {{day}} 天",
    "chapterProgress": "第 {{current}} 个，共 {{total}} 个",
    "nextChapterCta": "下一章",
    "completeDayCta": "完成今日阅读",
    "completeSessionCta": "完成本时段",
    "listenChapterCounted": "{{reference}} 计入今天的计划",
    "dailyTargetCompleteTitle": "每日阅读完成",
    "dailyTargetCompleteBody": "您已经完成了今天的计划阅读。我们将带您回到您的计划，以便您可以看到您的进度。",
    "bibleIn1Year": {
      "title": "一年圣经",
      "description": "每天阅读旧约和新约，在 365 天内通读整本圣经。"
    },
    "newTestament90": {
      "title": "90天内读懂新约",
      "description": "只需 90 天即可浏览新约圣经。"
    },
    "psalms30": {
      "title": "30天内的诗篇",
      "description": "在一个月的每日阅读中阅读全部 150 篇诗篇。"
    },
    "gospels60": {
      "title": "60 天的福音书",
      "description": "用两个月的时间读完马太福音、马可福音、路加福音和约翰福音。"
    },
    "proverbs31": {
      "title": "每日箴言",
      "description": "按今天的日期阅读对应的箴言章节，每月重复。"
    },
    "kathisma": {
      "title": "Kathisma",
      "description": "阅读每个工作日早晚指定的诗篇分组，每周重复。"
    },
    "chronological": {
      "title": "按时间顺序排列的圣经",
      "description": "按照事件发生的顺序阅读圣经，从创世记到启示录。"
    },
    "epistles30": {
      "title": "30天读完书信",
      "description": "用一个月的时间读完保罗、彼得、雅各和约翰的书信。"
    },
    "sermonMount7": {
      "title": "登山宝训",
      "description": "为期一周的深入研究耶稣在马太福音 5-7 章中最著名的教导。"
    },
    "plans": "计划",
    "addFirstPlan": "添加您的第一个计划",
    "completeDayHint": "标志着今天的阅读计划完成",
    "completeSessionHint": "将此祷告时段标记为已完成",
    "noPlanSearchResults": "未找到匹配的计划",
    "rhythms": "节奏",
    "createRhythm": "创造节奏",
    "editRhythm": "编辑节奏",
    "manageRhythm": "管理节奏",
    "morningRhythm": "早晨节奏",
    "afternoonRhythm": "午后节奏",
    "eveningRhythm": "傍晚节奏",
    "morningLabel": "早晨",
    "middayLabel": "中午",
    "afternoonLabel": "午后",
    "eveningLabel": "傍晚",
    "sessionDone": "已完成",
    "sessionNext": "下一项",
    "sessionUpcoming": "即将开始",
    "noRhythms": "还没有节奏",
    "noRhythmsBody": "通过将计划进度与您可以随时重新排序的自定义段落混合起来，构建可重复的流程。",
    "rhythmName": "节奏名称",
    "rhythmNamePlaceholder": "将自动名称留空",
    "rhythmPlanCount": "{{count}} 个计划",
    "rhythmItemCount": "{{count}} 项",
    "includedPlans": "包含的读经计划",
    "includedItems": "包含项目",
    "availablePlans": "可用的读经计划",
    "rhythmsIntro": "通过将计划进度与自定义经文段落混合起来，构建可重复的早晨、下午或晚上的流程。",
    "rhythmSlotPrompt": "先选择你要构建的流程类型。",
    "rhythmSlotActive": "已选择 {{name}}",
    "selected": "已选择",
    "continueRhythm": "继续节奏",
    "nextUp": "下一项：{{value}}",
    "openRhythm": "打开节奏",
    "deleteRhythm": "删除节奏",
    "emptyRhythmPlans": "至少添加一个计划才能保存此节奏。",
    "emptyRhythmItems": "至少添加一个计划或经文段落才能保存此节奏。",
    "alreadyInAnotherRhythm": "这个计划已经是另一种节奏了。",
    "planUnavailableForRhythm": "只有注册的、活动的计划才能添加到节奏中。",
    "rhythmDaySummary": "当前日期：{{day}}",
    "rhythmSequence": "节奏序列",
    "rhythmPlanSource": "{{count}} 天计划",
    "rhythmPassageLabel": "{{count}} 章",
    "planItemLabel": "计划",
    "passageItemLabel": "经文段落",
    "addPassage": "添加经文段落",
    "addPassageToRhythm": "将经文加入节奏",
    "repeatablePassage": "可重复阅读的经文",
    "startChapterLabel": "开始章节",
    "endChapterLabel": "结束章节",
    "maxChapterHint": "已提供 {{count}} 个章节",
    "chooseBook": "首先选择一本书。",
    "chapterCount": "{{count}} 章",
    "deleteRhythmConfirmTitle": "删除这个节奏？",
    "deleteRhythmConfirmBody": "包含的计划将保留在“我的计划”中，但此节奏将被删除。",
    "foundationsOfTheGospel": {
      "title": "福音的基础",
      "description": "在 14 天内追溯从创造到新创造的救赎故事。"
    },
    "prayerIntimacyWithGod": {
      "title": "祷告与亲近神",
      "description": "为期 7 天的祷告、敬拜和与主亲密交通的旅程。"
    },
    "morningAndEveningPrayer": {
      "title": "晨祷与晚祷",
      "description": "以诗篇、福音书和晚间经文开始与结束每一天。"
    },
    "prayerOfTheHours": {
      "title": "按时祷告",
      "description": "以诗篇、福音书和书信构成早晨、午间及晚间的阅读。"
    },
    "identityInChrist": {
      "title": "在基督里的身份",
      "description": "阅读将你的身份、安全感和呼召植根于耶稣的经文。"
    },
    "kingdomOfGod": {
      "title": "神的国度",
      "description": "探索福音书和早期教会中耶稣关于神国度的教导。"
    },
    "spiritualWarfare": {
      "title": "属灵争战",
      "description": "了解神如何装备他的子民，以坚定地抵抗属灵的反对。"
    },
    "holinessAndSanctification": {
      "title": "圣洁与成圣",
      "description": "通过有关圣洁、顺服和成长的章节，追求一种与众不同的生活。"
    },
    "greatCommissionAndMission": {
      "title": "大使命与使命",
      "description": "跟随上帝的使命，从对亚伯拉罕的应许一直到地极。"
    },
    "faithAndObedience": {
      "title": "信心与服从",
      "description": "通过忠实回应的故事和教导来加强信任和服从。"
    },
    "hearingGodsVoice": {
      "title": "聆听神的声音",
      "description": "通过先知、诗篇、耶稣和早期教会聆听上帝的声音。"
    },
    "categoryChronological": "按时间顺序",
    "categoryBookStudy": "书卷研读",
    "categoryTopical": "主题",
    "categoryDevotional": "灵修",
    "timedChallenges": "阅读挑战",
    "bibleIn30Days": {
      "title": "30 天读完完整圣经",
      "description": "在一个月内读完整本圣经——每天大约 40 章。终极圣经冲刺。"
    },
    "bibleIn90Days": {
      "title": "90 天内读完完整圣经",
      "description": "90天读完圣经全部1189章，每天约13章。"
    },
    "bibleIn6Months": {
      "title": "6 个月内读完完整圣经",
      "description": "在 180 天内按规律的节奏读完整本圣经，每天大约 7 章。"
    },
    "ntIn7Days": {
      "title": "7 天读懂新约",
      "description": "一周读完新约全部 260 章——每天大约 37 章。对于大胆的人来说。"
    },
    "ntIn14Days": {
      "title": "两周内读懂新约",
      "description": "14 天读完整本新约，每天大约 19 章。"
    },
    "ntIn30Days": {
      "title": "30天读懂新约",
      "description": "在一个月内读完整本新约——每天大约 9 章。"
    },
    "ntIn6Months": {
      "title": "六个月新约",
      "description": "用 180 天的时间慢慢读完新约 27 本书，每天大约 1-2 章。"
    },
    "gospels7Days": {
      "title": "7 天福音",
      "description": "在一周内阅读《马太福音》、《马可福音》、《路加福音》和《约翰福音》——每天大约 13 章。"
    },
    "gospels14Days": {
      "title": "两周内的福音",
      "description": "在 14 天内读完所有四福音书，每天大约 6 章。"
    },
    "gospels30Days": {
      "title": "30天内的福音",
      "description": "为期一个月的舒适旅程，浏览所有四福音书，每天大约 3 章。"
    },
    "psalms7Days": {
      "title": "7 天的诗篇",
      "description": "在紧张的一周内完成全部 150 篇诗篇——每天大约 21 篇诗篇进行敬拜和祈祷。"
    },
    "psalms90Days": {
      "title": "90 天的诗篇",
      "description": "在 90 天内慢慢吸收所有 150 篇诗篇——每天少于 2 篇诗篇。"
    },
    "otInYear": {
      "title": "一年读旧约",
      "description": "在365天内读完旧约全书929章，每天大约2-3章。"
    },
    "otIn90Days": {
      "title": "90 天读懂旧约",
      "description": "90天读完旧约全部929章，每天约10章，需要坚定的投入。"
    },
    "pentateuch30Days": {
      "title": "30 天读完摩西五经",
      "description": "从创世记到申命记——摩西的前五本书——在一个月内完成。"
    },
    "wisdom30Days": {
      "title": "30天智慧文学",
      "description": "约伯记、诗篇、箴言、传道书和所罗门之歌的 30 天反思。"
    },
    "prophets90Days": {
      "title": "90天内的先知",
      "description": "用90天专注阅读全部17卷先知书，从以赛亚书到玛拉基书。"
    },
    "paulsLetters30Days": {
      "title": "保罗三十天内的书信",
      "description": "在一个月内读完保罗的全部 13 封书信——罗马书到腓利门书。"
    },
    "acts28Days": {
      "title": "28天读完使徒行传",
      "description": "每天一章使徒行传——跟随早期教会从耶路撒冷到罗马。"
    },
    "revelation22Days": {
      "title": "22天内的启示",
      "description": "每天一章启示录——以稳定的速度打开最后一卷圣经。"
    }
  },
  "annotations": {
    "title": "我的笔记与经文标记",
    "bookmarks": "书签",
    "highlights": "亮点",
    "notes": "笔记",
    "addNote": "添加注释",
    "addBookmark": "书签",
    "highlight": "强调",
    "editNote": "编辑备注",
    "deleteAnnotation": "删除",
    "noAnnotations": "还没有注释",
    "noBookmarks": "还没有书签",
    "noHighlights": "还没有亮点",
    "noNotes": "还没有笔记",
    "noteHint": "写下你的想法...",
    "saved": "已保存",
    "colors": {
      "yellow": "黄色的",
      "green": "绿色的",
      "blue": "蓝色的",
      "pink": "粉色的",
      "orange": "橙子",
      "red": "红色的"
    },
    "selected": "已选择",
    "note": "笔记",
    "copy": "复制",
    "removeHighlight": "删除突出显示"
  },
  "groups": {
    "loadingGroup": "正在加载组...",
    "groupNotFound": "找不到组",
    "unableToLoadGroup": "无法加载组。",
    "goBack": "回去",
    "joinCode": "加入代码",
    "share": "分享",
    "syncedProgress": "启用会话推出后，将显示同步进度。",
    "lessonsCompleted_one": "{{count}} 课已完成",
    "lessonsCompleted_other": "已完成 {{count}} 节课程",
    "nextLesson": "下一个：{{title}}",
    "startGroupSession": "开始小组会议",
    "saveSyncedSession": "保存同步会话",
    "syncedGroupPreview": "同步组预览",
    "syncedGroupPreviewBody": "在后端配置和登录访问都为此构建准备就绪之前，安全同步会话记录不可用。",
    "readOnlySyncedMembership": "只读同步会员资格",
    "readOnlySyncedMembershipBody": "此同步群组来自您登录的帐户。随着推出的继续，成员姓名和同步的课程历史记录将显示在此处。",
    "members": "成员 ({{count}})",
    "you": "（你）",
    "leader": "领导者",
    "joinedRecently": "最近加入",
    "joinedDate": "已加入 {{date}}",
    "leaveGroup": "离开群组",
    "leaveGroupLeaderMessage": "作为领导者，离开会将领导权转移给下一个最年长的成员。你确定吗？",
    "leaveGroupMemberMessage": "您确定要离开该群组吗？",
    "leave": "离开",
    "session": {
      "title": "小组会议",
      "lookBack": "回顾",
      "lookUp": "抬头",
      "lookForward": "期待",
      "duration5min": "〜5分钟",
      "duration10min": "〜10分钟",
      "saving": "保存...",
      "completeSession": "完整的会话",
      "syncedNotice": "完成此会话将为您登录的组保存同步记录。",
      "lookBackDescription": "首先检查每个人如何应用上周的课程。这建立了责任感并庆祝服从。",
      "lookBackQ1": "你如何遵守上次所学的内容？",
      "lookBackQ2": "你和谁分享的？发生了什么？",
      "lookBackQ3": "您面临哪些挑战？",
      "discussTogether": "一起讨论：",
      "lookBackTip": "庆祝胜利！鼓励那些服从的人。温和地鼓励那些没有在本周再次尝试的人。",
      "lookUpDescription": "一起阅读圣经并发现上帝的教导。",
      "discoveryQuestions": "发现问题：",
      "lookUpQ1": "这段经文教导了有关神的什么？",
      "lookUpQ2": "它对人有什么教导？",
      "lookUpQ3": "是否有可以遵循或避免的例子？",
      "lookUpQ4": "有命令要遵守吗？",
      "lessonQuestions": "课程问题：",
      "lookForwardDescription": "承诺遵守您所学到的知识并与他人分享。",
      "keyTakeaway": "要点：",
      "thisWeeksPractice": "本周练习：",
      "commitTogether": "共同承诺：",
      "lookForwardQ1": "“我会……” - 你将如何遵守这个教导？",
      "lookForwardQ2": "“我将与......分享” - 你会教谁这个？",
      "closingPrayer": "密切祈祷。求上帝帮助每个人服从并分享他们所学到的东西。为他们将与之分享的人祈祷。"
    },
    "syncSession": {
      "unavailableTitle": "同步会话不可用",
      "backendUnavailable": "此版本尚未配置同步组录制。",
      "signInRequired": "您必须先登录才能保存同步的群组会话。",
      "saveFailedTitle": "无法保存同步会话",
      "saveFailedDefault": "无法保存同步的组会话。"
    }
  },
  "prayer": {
    "title": "祈祷墙",
    "submitRequest": "分享祷告请求",
    "requestPlaceholder": "你想为什么祷告？",
    "prayed": "祈祷了",
    "encouraged": "鼓励",
    "answered": "已回答",
    "markAnswered": "标记为已答复",
    "noPrayers": "还没有祈祷请求",
    "beFirst": "成为第一个与你的小组分享祈祷请求的人。",
    "prayedCount": "{{count}} 已祈祷",
    "encouragedCount": "鼓励 {{count}}",
    "you": "你",
    "groupMember": "团体成员",
    "signInTitle": "登录并与该小组一起祈祷",
    "signInBody": "登录以分享请求、与其他人一起祈祷并使该群组保持同步。",
    "ownerLongPressHint": "长按可编辑或删除"
  },
  "engagement": {
    "title": "你的旅程",
    "chaptersRead": "已读章节",
    "listeningTime": "聆听时间",
    "currentStreak": "当前连胜",
    "longestStreak": "最长连续纪录",
    "plansCompleted": "计划已完成",
    "engagementScore": "参与度分数",
    "annotationsCreated": "笔记与经文标记",
    "minutes": "分钟",
    "hours": "小时",
    "days": "天",
    "totalChapters": "总章节数",
    "totalListening": "总收听时长"
  },
  "translations": {
    "pin": "添加到我的译本",
    "unpin": "取消置顶",
    "hide": "从我的译本中隐藏",
    "title": "圣经翻译",
    "languagePreference": "语言偏好",
    "myTranslations": "我的翻译",
    "primary": "初级翻译",
    "secondary": "比较翻译",
    "audioPreference": "音频翻译",
    "installed": "已安装",
    "available": "可用的",
    "bundled": "捆绑式",
    "download": "下载",
    "publicDomain": "公共领域",
    "cloudLibrary": "云图书馆",
    "downloading": "正在下载...",
    "downloadPrompt": "下载 {{name}} 的经文文本（约 {{size}} MB），以便离线阅读？",
    "deleteConfirmTitle": "删除翻译",
    "deleteConfirmMessage": "删除 {{name}} 的下载数据吗？您可以随时重新下载。",
    "cancelDownload": "取消下载",
    "delete": "删除"
  },
  "gather": {
    "complete": "完成",
    "discoveryBibleStudy": "发现式查经",
    "foundationsSummary": "{{foundations}} 个根基 · {{lessons}} 节课程",
    "lessonOfCount": "第 {{number}} 课，共 {{total}} 课",
    "upNextLesson": "接下来 · 第 {{lesson}} 课，共 {{total}} 课",
    "upNextSubtitle": "{{reference}} · {{parent}}",
    "title": "聚集",
    "foundations": "根基",
    "wisdom": "智慧",
    "infoBanner": "帮助他人开始自己在圣经中认识神的旅程。",
    "wisdomBanner": "如果你的小组有特定的需求，在回到根基或训练课程之前，先做一些专题课程。",
    "getStarted": "从根基开始",
    "foundationLabel": "根基 {{number}}",
    "lessonsProgress": "{{completed}}/{{total}}",
    "showMore": "显示更多",
    "showLess": "显示更少",
    "sendInvitation": "发送邀请",
    "gatherWithOthers": "与他人聚集",
    "invitationDescription": "邀请某人一起探索圣经。",
    "upNext": "接下来",
    "fellowship": "团契",
    "story": "故事",
    "application": "应用",
    "shareAudio": "分享课程音频",
    "shareText": "分享课程文字",
    "shareLink": "分享课程链接",
    "download": "下载",
    "removeDownload": "删除下载",
    "markComplete": "标记为已完成",
    "completed": "已完成",
    "markIncomplete": "标记为未完成",
    "manageBookmarks": "管理书签",
    "lessonComplete": "课程已完成",
    "readTheStory": "阅读故事 →",
    "fellowshipQ1": "根据自上次聚会以来发生在你身上的事，你有什么感恩的事？",
    "fellowshipQ2": "这周有什么让你感到压力，你需要什么让事情变得更好？",
    "fellowshipQ3": "你社区里的人有哪些需要，我们如何互相帮助来满足我们所表达的需要？",
    "fellowshipQ4": "现在，让我们来读今天来自神的故事……",
    "applicationQ1": "现在，让某人用自己的话重述这段经文，就像在告诉一个从未听过的朋友。如果有遗漏或错误添加，我们来帮助他们。如果发生这种情况，我们可以问：\"你在故事中哪里找到这个？\"",
    "applicationQ2": "这个故事告诉我们关于神、祂的品格和祂所做的事情是什么？",
    "applicationQ3": "从这个故事中，我们对人，包括我们自己，学到了什么？",
    "applicationQ4": "这周你将如何在生活中应用这个故事中神的真理？你将采取的具体行动或事情是什么？",
    "applicationQ5": "在我们再次聚会之前，你将与谁分享这个故事中的一个真理？你知道有其他人也想像我们一样在这个应用中发现神的话语吗？",
    "applicationQ6": "当我们的聚会接近尾声时，让我们决定何时再次聚会以及谁来主持我们的下一次聚会。",
    "applicationQ7": "我们鼓励你记下你说要做的事，并在我们下次聚会之前的日子里重读这个故事。如果有人没有，主持人可以分享故事的文字或音频。当我们前行时，让我们求主帮助我们。",
    "foundation1Title": "神的故事",
    "foundation1Desc": "从创造到耶稣的诞生",
    "foundation2Title": "耶稣的生命与事工",
    "foundation2Desc": "从祂最初的日子到复活和升天",
    "foundation3Title": "福音的邀请",
    "foundation3Desc": "耶稣是谁，祂提供什么，以及如何回应",
    "foundation4Title": "门徒的生命",
    "foundation4Desc": "在顺服、祷告和属灵习惯中成长",
    "foundation5Title": "耶稣群体的生命",
    "foundation5Desc": "一起归属，一起服事，一起敬拜",
    "foundation6Title": "领袖的生命",
    "foundation6Desc": "培育门徒并领导他人",
    "foundation7Title": "分享好消息",
    "foundation7Desc": "带着福音去接触他人",
    "lessons": {
      "f101": "神创造世界",
      "f102": "人类陷入罪中",
      "f103": "神审判破碎的世界",
      "f104": "亚伯拉罕信靠并顺服神",
      "f105": "神拯救祂的子民：逾越节",
      "f106": "神颁布祂的律法",
      "f107": "悖逆的循环",
      "f108": "受苦的仆人",
      "f109": "耶稣的诞生",
      "f201": "耶稣开始祂的事工",
      "f202": "耶稣医治并赦免",
      "f203": "耶稣的能力",
      "f204": "爱神爱人",
      "f205": "神寻找失丧的人",
      "f206": "耶稣拥有胜过死亡的权柄",
      "f207": "耶稣服事：最后的晚餐",
      "f208": "钉十字架",
      "f209": "复活与升天",
      "f301": "耶稣是谁？",
      "f302": "耶稣提供什么？",
      "f303": "信靠耶稣的结果",
      "f304": "跟随耶稣的代价",
      "f305": "回应耶稣",
      "f306": "洗礼的意义",
      "f307": "耶稣对跟随者的应许",
      "f308": "圣灵如何帮助我们",
      "f309": "逼迫：当有的期待",
      "f310": "耶稣最后的命令",
      "f401": "门徒爱神爱人",
      "f402": "门徒是神的儿女",
      "f403": "门徒在祷告中寻求神",
      "f404": "门徒靠神的话生活",
      "f405": "门徒被圣灵引导",
      "f406": "门徒住在耶稣里",
      "f407": "门徒顺服并得人如鱼",
      "f408": "门徒脱去旧人穿上新人",
      "f409": "门徒认罪并饶恕他人",
      "f410": "门徒穿上神的全副军装",
      "f501": "教会是基督的身体",
      "f502": "教会是神的家和殿",
      "f503": "教会是合一的",
      "f504": "教会赞美神",
      "f505": "教会彼此服事",
      "f506": "教会一起祷告",
      "f507": "教会慷慨给予",
      "f508": "教会纪念基督的牺牲",
      "f509": "教会处理罪",
      "f510": "教会差遣工人出去",
      "f601": "耶稣：仆人式领袖",
      "f602": "好牧人与坏牧人",
      "f603": "好领袖做什么",
      "f604": "领袖资格",
      "f605": "领袖装备和倍增",
      "f606": "领袖为他们的人祷告",
      "f607": "领袖寻求神的智慧",
      "f608": "领袖是忠心的管家",
      "f609": "领袖在艰难中坚持",
      "f610": "领袖放弃自己的权利",
      "f701": "耶稣呼召得人如鱼的渔夫",
      "f702": "庄稼多",
      "f703": "奉耶稣的权柄去",
      "f704": "圣灵赋予我们见证的能力",
      "f705": "向所有人作见证",
      "f706": "福音跨越文化界限",
      "f707": "跨文化传递福音",
      "f708": "教会植立新群体",
      "f709": "万国都要赞美神"
    },
    "categoryInnerLife": "内心生命",
    "categoryChallenge": "挑战",
    "categoryMoney": "金钱",
    "categoryPeople": "人际关系",
    "categoryKnowingGod": "认识神",
    "topicCourage": "勇气",
    "topicFaith": "信心",
    "topicHope": "盼望",
    "topicJustice": "公义",
    "topicLove": "爱",
    "topicObedience": "顺服",
    "topicAnger": "愤怒",
    "topicCrisis": "危机",
    "topicGrief": "悲伤",
    "topicHurt": "伤痛",
    "topicReconciliation": "和好",
    "topicKnownAndLoved": "被认识与被爱",
    "topicStress": "压力",
    "topicMoneyAndGod": "金钱与神",
    "topicMoneyAdvice": "理财建议",
    "topicGiving": "奉献",
    "topicFaithfulAtWork": "职场忠心",
    "topicMarriage": "婚姻",
    "topicMen": "男性",
    "topicParenting": "育儿",
    "topicSingles": "单身",
    "topicWomen": "女性",
    "topicYouth": "青年",
    "topicCharacterOfGod": "神的性情",
    "topicPromisesOfGod": "神的应许",
    "topicNamesOfGod": "神的名字",
    "wisdomLessons": {
      "courage": {
        "l1": "刚强壮胆",
        "l2": "大卫与歌利亚",
        "l3": "以利亚独自站立",
        "l4": "三个朋友在火中",
        "l5": "但以理在狮子坑",
        "l6": "以斯帖到王面前",
        "l7": "彼得和约翰在议会前",
        "l8": "保罗：活着是基督，死了是得益处"
      },
      "faith": {
        "l1": "亚伯拉罕信神",
        "l2": "百夫长的信心",
        "l3": "摸耶稣的女人",
        "l4": "能移山的信心",
        "l5": "多马看见就信了",
        "l6": "因信得救的恩典",
        "l7": "信心与行为",
        "l8": "信心的英雄"
      },
      "hope": {
        "l1": "神更新等候祂的人",
        "l2": "枯骨的山谷",
        "l3": "盼望不叫人羞耻",
        "l4": "苦难中的盼望",
        "l5": "活泼的盼望",
        "l6": "复活的盼望",
        "l7": "灵魂的锚",
        "l8": "荣耀的盼望"
      },
      "justice": {
        "l1": "公义与怜悯",
        "l2": "让公义如江河涌流",
        "l3": "为软弱者和孤儿伸张正义",
        "l4": "真正的禁食：解开不义的绳索",
        "l5": "坚持不懈的寡妇和不义的官",
        "l6": "不要偏待人",
        "l7": "财主与拉撒路",
        "l8": "神悦纳的年"
      },
      "love": {
        "l1": "神如此爱世人",
        "l2": "爱你们的仇敌",
        "l3": "最大的诫命",
        "l4": "你们要彼此相爱，像我爱你们一样",
        "l5": "没有什么能叫我们与神的爱隔绝",
        "l6": "爱是恒久忍耐，爱是恩慈",
        "l7": "神就是爱",
        "l8": "浪子回家受欢迎"
      },
      "obedience": {
        "l1": "神呼召年轻的撒母耳",
        "l2": "听命胜于献祭",
        "l3": "顺服的祝福",
        "l4": "聪明人与愚拙人盖房",
        "l5": "你们若爱我，就当遵守我的命令",
        "l6": "顺从神不顺从人",
        "l7": "要行道，不要单单听道",
        "l8": "基督的顺服"
      },
      "anger": {
        "l1": "该隐的怒气导致杀人",
        "l2": "摩西愤怒击打磐石",
        "l3": "扫罗对大卫的嫉妒和愤怒",
        "l4": "约拿对神怜悯的愤怒",
        "l5": "耶稣赶出圣殿里做买卖的人",
        "l6": "要慢慢地发怒",
        "l7": "不可含怒到日落",
        "l8": "要为咒诅你们的人祝福"
      },
      "crisis": {
        "l1": "约伯失去一切",
        "l2": "以利亚精疲力竭逃跑",
        "l3": "神是我们的避难所和力量",
        "l4": "门徒在风暴中",
        "l5": "耶稣在客西马尼",
        "l6": "保罗和西拉在监狱里",
        "l7": "神的平安",
        "l8": "神恢复约伯"
      },
      "grief": {
        "l1": "约伯在苦难中呼喊",
        "l2": "大卫为扫罗和约拿单哀歌",
        "l3": "耶和华靠近伤心的人",
        "l4": "悲伤中的盼望",
        "l5": "耶稣在拉撒路的坟墓前哭泣",
        "l6": "你们的忧愁要变为喜乐",
        "l7": "神在我们的一切患难中安慰我们",
        "l8": "神要擦去所有眼泪"
      },
      "hurt": {
        "l1": "约瑟被兄弟们出卖",
        "l2": "大卫被押沙龙背叛",
        "l3": "耶和华医治伤心的人",
        "l4": "耶利米的哀歌",
        "l5": "耶稣在家乡被拒绝",
        "l6": "司提反被石头打死",
        "l7": "保罗身上的刺",
        "l8": "要将你们一切的忧虑卸给神"
      },
      "reconciliation": {
        "l1": "以扫饶恕雅各",
        "l2": "约瑟饶恕他的兄弟",
        "l3": "献礼物前先与人和好",
        "l4": "无情的仆人",
        "l5": "浪子回家受欢迎",
        "l6": "神借着基督使我们与祂和好",
        "l7": "要以恩慈相待，彼此饶恕",
        "l8": "主怎样饶恕了你们，你们也要这样饶恕人"
      },
      "selfEsteem": {
        "l1": "神按自己的形象创造人",
        "l2": "耶和华是我的牧者",
        "l3": "神完全了解我",
        "l4": "耶稣欢迎小孩子",
        "l5": "耶稣呼叫撒该的名字",
        "l6": "神拣选了软弱的来叫强壮的羞愧",
        "l7": "我们是神的杰作",
        "l8": "我们在这一切事上已经得胜有余"
      },
      "stress": {
        "l1": "不要为明天忧虑",
        "l2": "凡劳苦担重担的人，可以到我这里来",
        "l3": "神在旷野供应以利亚",
        "l4": "祢必保守他心意坚定，得享平安",
        "l5": "神是我的避难所",
        "l6": "要专心仰赖耶和华",
        "l7": "超越人所能了解的平安",
        "l8": "要将你们一切的忧虑卸给神"
      },
      "moneyAndGod": {
        "l1": "你们不能又事奉神，又事奉玛门",
        "l2": "无知的富人",
        "l3": "少年富人",
        "l4": "敬虔加上知足是大得益处",
        "l5": "对富人的警告",
        "l6": "地和其中所充满的都属耶和华",
        "l7": "财富是不确定的",
        "l8": "财富不能拯救"
      },
      "moneyAdvice": {
        "l1": "勤劳的工人",
        "l2": "智慧与财富",
        "l3": "诚实的商业交往",
        "l4": "美名胜过大财",
        "l5": "先算代价",
        "l6": "十个仆人的比喻",
        "l7": "偿还所欠的",
        "l8": "无论在什么状况都知足"
      },
      "giving": {
        "l1": "亚伯拉罕将十分之一献给麦基洗德",
        "l2": "送来全部的十分之一",
        "l3": "寡妇的奉献",
        "l4": "你们要给人，就必有给你们的",
        "l5": "早期教会分享一切",
        "l6": "马其顿教会慷慨奉献",
        "l7": "神爱捐得乐意的人",
        "l8": "慷慨的心灵将会兴旺"
      },
      "marketplace": {
        "l1": "约瑟诚实地管理",
        "l2": "经商的才德妇人",
        "l3": "工人得工价是应当的",
        "l4": "帐棚匠保罗",
        "l5": "但以理在王的服务中出类拔萃",
        "l6": "不可偷盗；要亲手做工",
        "l7": "要做就当作，像是为主做的",
        "l8": "凡你们所做的，都不要发怨言"
      },
      "marriage": {
        "l1": "神创造男人和女人彼此相配",
        "l2": "才德的妇人",
        "l3": "爱人的专一",
        "l4": "路得的忠心",
        "l5": "婚姻誓约与忠实",
        "l6": "在基督里的丈夫与妻子",
        "l7": "彼此相爱、尊重和顺服",
        "l8": "羔羊婚宴"
      },
      "men": {
        "l1": "神创造男人来工作和照料",
        "l2": "约瑟：压力下的诚信",
        "l3": "大卫：合神心意的人",
        "l4": "好父亲",
        "l5": "像基督爱教会那样爱妻子的丈夫",
        "l6": "要打那美好的信仰之仗",
        "l7": "要坚守信仰",
        "l8": "有福的人"
      },
      "parenting": {
        "l1": "教导你的孩子",
        "l2": "哈拿将撒母耳献给神",
        "l3": "教养孩童走当行的道",
        "l4": "用爱心管教",
        "l5": "耶稣与孩子",
        "l6": "浪子的父亲",
        "l7": "提摩太传承的信仰",
        "l8": "你们做父亲的，不要惹儿女的气"
      },
      "singles": {
        "l1": "路得：忠心而有目标",
        "l2": "独身的恩赐",
        "l3": "耶和华是我的牧者和产业",
        "l4": "先求神的国",
        "l5": "万事都有定时",
        "l6": "神使孤独的人有家可归",
        "l7": "女先知亚拿独自事奉神",
        "l8": "无论在什么状况都知足"
      },
      "women": {
        "l1": "神创造女人为平等的形象承载者",
        "l2": "底波拉领导以色列",
        "l3": "才德的妇人",
        "l4": "马利亚的赞歌",
        "l5": "伯大尼的马利亚选择好的那份",
        "l6": "复活时的女性",
        "l7": "百基拉教导亚波罗",
        "l8": "被圣灵赐予恩赐并充满的女性"
      },
      "youth": {
        "l1": "神呼召年轻的撒母耳",
        "l2": "年轻的牧童大卫战胜歌利亚",
        "l3": "但以理和他的朋友们站立",
        "l4": "十二岁的耶稣：在父的事上",
        "l5": "在年幼时记念你的创造主",
        "l6": "少年富人",
        "l7": "不可叫人小看你年轻",
        "l8": "年轻的约西亚寻求神"
      },
      "characterOfGod": {
        "l1": "耶和华：有怜悯有恩典，不轻易发怒",
        "l2": "神是圣洁的",
        "l3": "在神没有难成的事",
        "l4": "耶和华是我的牧者",
        "l5": "神是无所不知、无所不在的",
        "l6": "神是公义正直的",
        "l7": "神就是爱",
        "l8": "神是信实的"
      },
      "promisesOfGod": {
        "l1": "与亚伯拉罕立约",
        "l2": "神应许有盼望有未来",
        "l3": "神应许赐新心",
        "l4": "神应许弥赛亚",
        "l5": "神的一切应许在基督里都是是的",
        "l6": "神必在你们身上成全祂所做的事",
        "l7": "没有什么能拦阻神的旨意",
        "l8": "神要使万物更新"
      },
      "namesOfGod": {
        "l1": "我是自有永有的",
        "l2": "耶和华以勒——神的供应",
        "l3": "耶和华拉法——神的医治",
        "l4": "耶和华尼西——神是我们的旌旗",
        "l5": "耶和华罗以——神是我们的牧者",
        "l6": "耶和华是迦——神是我们的公义",
        "l7": "耶和华沙龙——神是我们的平安",
        "l8": "以马内利——神与我们同在"
      }
    },
    "moreOptions": "更多选择"
  },
  "harvestStudies": {
    "christologyTitle": "基督论",
    "christologyDesc": "关于耶稣是谁、祂的使命、死亡、复活和统治的深入章节研究。",
    "gospelTitle": "福音",
    "gospelDesc": "关于罪、恩典、称义和借着基督得救的章节研究。",
    "prayerTitle": "祷告",
    "prayerDesc": "加强日常祷告生活的模式和教导。",
    "loveAndChristianLivingTitle": "爱与基督徒生活",
    "loveAndChristianLivingDesc": "在爱、圣洁和圣灵引导的顺服中行走的实践章节。",
    "discipleshipTitle": "门徒训练",
    "discipleshipDesc": "关于以顺服、降服和国度优先跟随耶稣的教导。",
    "churchTitle": "教会",
    "churchDesc": "教会的身份、合一和使命在行动中的体现。",
    "missionHarvestTitle": "宣教 / 收割",
    "missionHarvestDesc": "关于传福音、见证和大使命的核心章节。",
    "groupEternalChrist": "永恒的基督（先存）",
    "groupBirthOfJesus": "耶稣的诞生（道成肉身）",
    "groupMissionOfJesus": "耶稣的使命",
    "groupDeathOfJesus": "耶稣的死（赎罪）",
    "groupResurrectionOfJesus": "耶稣的复活",
    "groupExaltedChrist": "被高举的基督",
    "groupGospelSalvation": "福音（救恩）",
    "groupPrayerCore": "祷告",
    "groupLoveAndLivingCore": "爱与基督徒生活",
    "groupDiscipleshipCore": "门徒训练",
    "groupChurchCore": "教会",
    "groupMissionHarvestCore": "宣教 / 收割"
  },
  "learn": {
    "sectionTabs": "课程部分",
    "decreaseTextSize": "缩小文字",
    "increaseTextSize": "放大文字",
    "playbackAndText": "播放与文本",
    "playbackSpeed": "播放速度",
    "fontSizeLabel": "字体大小",
    "noPassageText": "没有可用的经文段落文本",
    "listenToStoryAgain": "再次聆听故事",
    "shareApp": "分享应用",
  },
  "plans": {
    "rhythmComposer": {
      "subtitle": "精选的节奏库，二十个历史起点，一键即可添加。",
      "heroEyebrow": "历史节奏",
      "heroTitle": "从真实的传统开始，而不是从空白表单开始",
      "heroBody": "天主教、圣公会、东正教、本笃会、路德宗、清教徒以及受泰泽启发的节奏，已经塑造成可重复的经文段落，适用于早晨、中午、傍晚或任何时间。",
      "presetCount": "{{count}} 个预设",
      "prayerAndScripture": "祷告与经文",
      "tapToAdd": "点击添加",
      "replaceCurrentTitle": "替换当前节奏",
      "replaceCurrentBody": "在下方选择一个预设以完全替换此节奏。这能让流程保持简单，并使每个节奏都从清晰的传统开始。",
      "timeOfDay": "时段",
      "filterAll": "全部",
      "midday": "正午",
      "anyTime": "任何时间",
      "tradition": "传统",
      "allTraditions": "所有传统",
      "historicRoots": "历史渊源",
      "includes": "包含",
      "emptyTitle": "没有节奏与这些筛选条件匹配。",
      "emptyBody": "尝试更广泛的传统或一天中的不同时段。",
      "rhythmNotFound": "未找到节奏。",
      "addRhythm": "添加节奏",
      "replaceRhythm": "替换节奏",
      "errorEmptyItems": "至少选择一项来构建此节奏。",
      "errorPlanInAnotherRhythm": "此计划已属于另一个节奏。",
      "errorRhythmNotFound": "找不到该节奏。",
    }
  }
} as const;
