...
test('the Proverbs ledger excludes dates that do not exist in the current month', () => {
  ...
  const monthLengths: Array<[year: number, month: number, days: number]> = [
    [2024, 1, 29],
    [2025, 1, 28],
    [2026, 8, 30],
    [2026, 9, 31],
...
test('reconcileFetchedPlanProgress merges matching local and remote plan rows', () => {
  ...
  assert.ok(reconciled, 'the reconciled plan progress');
  assert.deepEqual(Object.keys(reconciled.completed_entries), ['1', '2']);
  assert.equal(reconciled.current_day, 3);
  assert.equal(reconciled.user_id, 'user-1');
});
