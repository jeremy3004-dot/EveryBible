...
test('usageQueue exposes the unified enqueue/flush/count API', () => {
  ...
  assert.match(
    source,
    /export function enqueueUsageEvent\s*\(/,
    'enqueueUsageEvent must be exported'
  );
  assert.match(
    source,
    /export async function flushUsageQueue\s*\(/,
    'flushUsageQueue must be exported'
  );
  assert.match(
    source,
    /export function getPendingUsageEventCount\s*\(/,
    'count helper must be exported'
  );
  assert.match(
    source,
    /export function generateUUID\s*\(/,
    'generateUUID must be exported for the facades'
  );
});
...
test('usageQueue attaches auth OPTIONALLY (token when present, none when absent)', () => {
  ...
  assert.match(
    source,
    /auth\.getSession\(\)/,
    'flush should read the current session for an optional token'
  );
  assert.match(
    source,
...
test('usageQueue enriches every batch with client geo before delivery', () => {
  ...
  assert.match(
    ...
    'queued events should be enriched with geo'
  );
});
...
test('usageQueue restores lazily and persists the full bounded pending queue', () => {
  const source = readRelativeSource('./usageQueue.ts');
  assert.match(source, /function ensureQueueRestored/);
  assert.match(source, /MAX_PERSISTED_EVENTS = MAX_QUEUE_SIZE/);
  assert.match(source, /mmkvInstance/);
});
