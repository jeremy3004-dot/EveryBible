...
// ---------------------------------------------------------------------------
// Attempt lifecycle: inactivity timeouts, retries, cancellation
//
// Every scenario drives the real `downloadAudioBook` through the injected
// file-system adapter, so the transport can be stalled, aborted, or made to
// resolve late without touching a real download.
// ---------------------------------------------------------------------------
...
test('directory failure marks the job failed and releases its cancellation controller', async (t) => {
  ...
  requestAudioDownloadCancellation('audio-download:bsb:book:PHM');
...
test('a transport that cannot be stopped ends the book download without another attempt', async (t) => {
  t.mock.timers.enable({ apis: ['setTimeout'] });
  const runtime = bookRuntime();
  let attempts = 0;
  runtime.fileSystem.downloadFile = async () => {
    attempts += 1;
    throw new AudioDownloadStopError('stop failed');
  };
  const rejected = assert.rejects(runtime.start(), /stop failed/i);
  await flush();
  t.mock.timers.tick(60_000);
  await flush();
