...
test('counts for a single request are aggregated by type', async () => {
  fake.respondTo('prayer_interactions', () => ({
    data: [{ type: 'prayed' }, { type: 'prayed' }, { type: 'encouraged' }],
  }));

  assert.deepEqual(await prayer.getInteractionCounts('req-1'), {
    success: true,
    data: { prayed: 2, encouraged: 1 },
  });
  const call = lastCall('prayer_interactions');
  assert.equal(call.columns, 'type');
  assert.deepEqual(filtersOf(call), [['request_id', 'req-1']]);
});

test('unrecognised interaction types are excluded from the counts', async () => {
  fake.respondTo('prayer_interactions', () => ({
    data: [{ type: 'prayed' }, { type: 'amened' }],
  }));

  assert.deepEqual(await prayer.getInteractionCounts('req-1'), {
    success: true,
    data: { prayed: 1, encouraged: 0 },
  });
});

test('a null counts payload reports zeroes', async () => {
  fake.respondTo('prayer_interactions', () => ({ data: null }));

  assert.deepEqual(await prayer.getInteractionCounts('req-1'), {
    success: true,
    data: { prayed: 0, encouraged: 0 },
  });
});

test('counts are zero without a backend so the UI can render offline', async () => {
  backend.configured = false;
...
test('counts are queried without an auth round trip because RLS already scopes them', async () => {
  fake.auth.setSession(null);
  fake.respondTo('prayer_interactions', () => ({ data: [{ type: 'prayed' }] }));
  ...
  assert.deepEqual(
    fake.authCalls,
    [],
    'a signed-out reader still gets counts for a request RLS lets them see'
  );
});

test('a rejected counts query surfaces the database error', async () => {
  fake.respondTo('prayer_interactions', () => ({
    data: null,
...
test('a network exception while counting is reported with its message', async () => {
  fake.respondTo('prayer_interactions', () => {
    throw new Error('Network request failed');
  });
...
test('a non-Error thrown while counting is reported as an unknown error', async () => {
  fake.respondTo('prayer_interactions', () => {
    throw 'boom';
  });
