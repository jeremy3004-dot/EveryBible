...
const outputRoot = path.join(sourceRoot, 'ipad-pro-129-2026-04-03');

const canvas = { width: 2048, height: 2732 };
const scaleX = canvas.width / 1290;
const scaleY = canvas.height / 2796;
...
function buildSlideSvg({ action, benefitLines, screenImage }) {
  const benefitText = benefitLines
    .map(
      (line, index) =>
        `<tspan x="${sx(645)}" dy="${index === 0 ? 0 : sf(90)}">${escapeXml(line)}</tspan>`
    )
    .join('');
    ...
    <svg width="${canvas.width}" height="${canvas.height}" viewBox="0 0 ${canvas.width} ${canvas.height}" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <filter id="phoneShadow" x="-20%" y="-20%" width="140%" height="160%">
          <feDropShadow dx="0" dy="${sy(46)}" stdDeviation="${sf(40)}" flood-color="#4B1711" flood-opacity="0.4" />
          <feDropShadow dx="0" dy="${sy(16)}" stdDeviation="${sf(18)}" flood-color="#000000" flood-opacity="0.25" />
        </filter>
        ...
        <clipPath id="screenClip">
          <rect x="${sx(189)}" y="${sy(553)}" width="${sx(912)}" height="${sy(1974)}" rx="${sy(110)}" ry="${sy(110)}" />
        </clipPath>
      </defs>
      ...
      <g filter="url(#phoneShadow)">
        <rect x="${sx(145)}" y="${sy(505)}" width="${sx(1000)}" height="${sy(2085)}" rx="${sy(126)}" ry="${sy(126)}" fill="url(#frameGloss)" />
        <rect x="${sx(159)}" y="${sy(519)}" width="${sx(972)}" height="${sy(2057)}" rx="${sy(118)}" ry="${sy(118)}" fill="#050505" />
        <rect x="${sx(179)}" y="${sy(539)}" width="${sx(932)}" height="${sy(2017)}" rx="${sy(112)}" ry="${sy(112)}" fill="${palette.screenBorder}" />
      </g>
      ...
      <rect x="${sx(470)}" y="${sy(584)}" width="${sx(350)}" height="${sy(66)}" rx="${sy(33)}" ry="${sy(33)}" fill="#090909" opacity="0.96" />
      <rect x="${sx(451)}" y="${sy(604)}" width="${sx(18)}" height="${sy(18)}" rx="${sy(9)}" ry="${sy(9)}" fill="#121212" opacity="0.85" />
      <rect x="${sx(830)}" y="${sy(604)}" width="${sx(18)}" height="${sy(18)}" rx="${sy(9)}" ry="${sy(9)}" fill="#121212" opacity="0.85" />

      <rect x="${sx(1119)}" y="${sy(870)}" width="${sx(8)}" height="${sy(290)}" rx="${sy(4)}" ry="${sy(4)}" fill="#3D3531" opacity="0.95" />
      <rect x="${sx(163)}" y="${sy(968)}" width="${sx(8)}" height="${sy(210)}" rx="${sy(4)}" ry="${sy(4)}" fill="#2E2825" opacity="0.92" />
      <rect x="${sx(163)}" y="${sy(1214)}" width="${sx(8)}" height="${sy(384)}" rx="${sy(4)}" ry="${sy(4)}" fill="#2E2825" opacity="0.92" />
    </svg>
  `;
}
...
async function main() {
  ...
  console.log(`Generated ${slides.length} iPad App Store scaffolds in ${outputRoot}`);
}
