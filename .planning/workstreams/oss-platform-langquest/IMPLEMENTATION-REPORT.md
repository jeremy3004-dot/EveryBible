...
- Trigger.dev deployment needs project ref and secret configured in the deployment environment.
...
2. Configure Trigger.dev project/env and deploy `apps/workflows`.
3. Configure LangQuest scoped read credentials in workflow environment.
4. Run discovery on one allowlisted project.
5. Review candidates in `/langquest`.
6. Mark one candidate as `ours`, select it, and run selected ingestion with a small chapter limit first.
7. Inspect R2 `langquest/ingest/...` artifacts and admin artifact rows.
8. Promote only after artifact validation and ownership approval.
9. Decide whether the listening app will consume segment manifests directly or whether workflows should generate merged chapter audio before `translation_catalog` exposure.
