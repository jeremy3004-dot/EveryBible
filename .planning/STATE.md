...
- Phase 35 EXECUTED + STATICALLY VERIFIED + MERGED TO MAIN (2026-06-12): All 6 fixes implemented (Sonnet executor) on `perf/android-hardening`, independently verified, and merged to `main`. Verification: `npm run typecheck` clean, `npm run lint` clean (1 introduced react/prop-types false-positive fixed via scoped disable), `npm run test:release` 528/528. Code-verified: P1, P3, P4, P5, P6. Device-QA plan: `notes/android-perf-device-qa-plan.md`
- Phase 35 DEVICE QA COMPLETE — PHASE FULLY VERIFIED (2026-06-12): Both gates passed on a real TECNO KL4 (Android 14, arm64). **Track B (P2 R8 minification): PASS — caught a real production-blocker:** the original P2 config FAILED at `minifyReleaseWithR8` because background-downloader's `com.eko` references `com.tencent.mmkv.MMKV`; fixed by adding `-dontwarn com.tencent.mmkv.**` to extraProguardRules (commit `013571c`, on main). Production AAB then built (versionCode 230) and cold-started clean (Hermes/ExoPlayer/Reanimated/MMKV all loaded; no FATAL/ClassNotFound/NoSuchMethod). **Track A (P1 follow-along): PASS** — verified by screenshot (highlight auto-advanced John 3:1→3:3 under audio; manual scroll + verse-select still update immediately; 0 JS errors in 2,627-line logcat). Evidence: `phases/35-android-performance-hardening/35-DEVICE-QA-REPORT.md` + untracked `qa-evidence/`. Note: QA AAB was built arm64-only for speed; the real all-arch release reuses the same architecture-independent R8 fix.
